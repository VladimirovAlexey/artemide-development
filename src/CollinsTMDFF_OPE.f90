...
public::CollinsTMDFF_OPE_tw3,CollinsTMDFF_OPE_tw3_resetGrid
...
subroutine CollinsTMDFF_OPE_Initialize(file,prefix)
    ...
    mainGrid=optGrid(path,'*18  ','*E   ',numberOfHadrons,withGluonTW3,moduleName,outputLevel)
...
!!! This is defining procedure it computes the convolution of tw3 FF with coefficient function
...
!!! this is convolution with twist3 FF
function CollinsTMDFF_OPE_tw3(x,b,h)
    real(dp),dimension(-5:5)::CollinsTMDFF_OPE_tw3
    real(dp),intent(in)::x,b
    integer,intent(in)::h

    if(useGridTW3) then
        CollinsTMDFF_OPE_tw3=mainGrid%Extract(x,b,h)
    else
        CollinsTMDFF_OPE_tw3=tw3_convolution(x,b,h)
    end if

end function CollinsTMDFF_OPE_tw3
...
!! call QCDinput to change the FF replica number
!! unset the grid, since it should be recalculated for different FF replica.
