...
function TMD_opt(x,bT,hadron)
  ...
  !!! test boundaries
    ...
    if(abs(hadron)<10) then
        ...
        TMD_opt=wglTMDPDF_OPE_tw2(x,bT,1)*FNP(x,bT,1,lambdaNP)&
        +wglTMDPDF_OPE_tw3(x,bT,1)*FNP_tw3(x,bT,1,lambdaNP)
...
function wglTMDPDF_TMM_G(x,mu,hadron)
    ...
    !!! forcefully set =0 below threshold
    if(mu<mBOTTOM) then
    wglTMDPDF_TMM_G(5)=0._dp
    wglTMDPDF_TMM_G(-5)=0._dp
    end if
    if(mu<mCHARM) then
    wglTMDPDF_TMM_G(4)=0._dp
    wglTMDPDF_TMM_G(-4)=0._dp
    end if
contains
    function F(b)
...
function wglTMDPDF_TMM_X(x,mu,hadron)
    ...
    !!! forcefully set =0 below threshold
    if(mu<mBOTTOM) then
    wglTMDPDF_TMM_X(5)=0._dp
    wglTMDPDF_TMM_X(-5)=0._dp
    end if
    if(mu<mCHARM) then
    wglTMDPDF_TMM_X(4)=0._dp
    wglTMDPDF_TMM_X(-4)=0._dp
    end if
contains
    function F(b)
