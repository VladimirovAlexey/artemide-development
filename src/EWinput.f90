...
!            arTeMiDe 3.04
!
! The module defining various QED and electroweak parameters
!
!    14.02.2019 values of ckm matrix are added AV.
!    26.06.2026 Some minor corrections. AV
!
!                        AV.  10.06.2018
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!-quark masses parameters (used to compute threshold of running alpha)
real(dp)::massCHARM,massBOTTOM,massTOP

!!alphaQED parameters
real(dp)::alphaZinv,alphaTAUinv !!! these are from constants-file
real(dp)::alphaELECTRONinv,alphaMUONinv,alphaCHARMinv,alphaBOTTOMinv,alphaTOPinv!!! these are calculated in Set_betaQED()
!! running parameters
real(dp)::betaQED !!! this is corrected beta function of qed matched to mZ and mTau thresholds with 1-loop evolution
...
!!!!alpha EM with 1-loop run
!!!! it is normalized at MZ and Mtau to the values specified in const-file (see also Set_betaQED())
pure function alphaEM(mu)
...
subroutine Set_EWconstants()
 ...
 !!!!  it is 2(gV^2+gA^2)/(8sw^2cw^2) for Z boson.
 
 !!!! paramMIX is given by
 !!!! eq(t3-2ef sW^2)/(2sw cW)
 !!!! eq*gV/(2sW cW)  for Z boson

 !!!! parameters z and r
 !!!! zP_gg = ef^2
 !!!! zP_gZ = eq(t3-2ef sW^2)/(2sw cW) = paramMIX
 ...
 paramL_A=(4*Abs(ef)*sW2-1)/(8*sW2*cW2)
...
!!! It uses alphaZ^-1 and alphaTau^-1 to determine the beta function of QED with thresholds
!!! the thresholds used
!!! e(+u+d); muon(+s); c; tau; b; t;
!!! betaEFF=Neff*beta
...
!!!
!!! It also computes threshold values of alpha QED
subroutine Set_betaQED()
  ...
  if(outputLevel>2) write(*,*) "    Effective QED beta function in fractions of LO:",deltaB
  ...
  if(outputLevel>2) write(*,*) "    Threshold values of alpha^(-1) QED (mE,mMU,mC,mTAU,mB,mT):"
