...
SELECT CASE(process(3))
  ...
  CASE (1) !pp->gamma
    ! e_q^2 *F_q(A)*F_qbar(B)
     ...
     FB=uTMDPDF_inKT(x2,sqrt(k2),mu,Q2,-h2) !!! -h2, to myltiply quarks by anti-quarks in FAB
     FAB=FA*FB
 ...
 !--------------------------------------------------------------------------------
  CASE (2,3,4,5,20,21,22) !Delta^{GG'}z_{+l}z_{+f}f1f1
     FA=uTMDPDF_inKT(x1,sqrt(k1),mu,Q2,h1)
     FB=uTMDPDF_inKT(x2,sqrt(k2),mu,Q2,-h2)!!! -h2, to multiply quarks by anti-quarks in FAB
     FAB=FA*FB
  ...
  CASE (8,9,10,30,31,32) !Delta^{GG'}z_{+l}r_{+f}h1h1
     FA=BoerMuldersTMDPDF_inKT(x1,sqrt(k1),mu,Q2,h1)
     FB=BoerMuldersTMDPDF_inKT(x2,sqrt(k2),mu,Q2,-h2)!!! -h2, to myltiply quarks by anti-quarks in FAB
     FAB=FA*FB
