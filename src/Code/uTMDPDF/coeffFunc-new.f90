...
!!!!! n= power of alphas
!!!!! k= power of log
pure function C_q_q_delta(n,k,Nf)
    ...
    CASE(2) !! NNLO
        SELECT CASE(k)
        CASE(0)
            C_q_q_delta=-2416d0/81d0-134d0/3d0*zeta2+448d0/9d0*zeta3+200d0/9d0*zeta4+&
                        Nf*(352d0/243d0+20d0/9d0*zeta2+56d0/27d0*zeta3)
        CASE(1)
            C_q_q_delta=(-14d0-140d0/3d0*zeta2+16d0/3d0*zeta3+Nf*(4d0/9d0+40d0/9d0*zeta2))
        CASE(2)
            C_q_q_delta=(-14d0+4d0/3d0*Nf-128d0/9d0*zeta2)
        CASE DEFAULT
            C_q_q_delta=0d0
        END SELECT
    ...
    -(244d0*zeta3**2)/3d0 + (12796d0*zeta4)/9d0  - (1576d0*zeta5)/3d0 - (59468d0*zeta6)/81d0
        ...
        CASE(2)
            C_q_q_delta=&
    (-302d0 + Nf**2*(-8d0/27d0 - (80d0*zeta2)/27d0) - (26476d0*zeta2)/27d0 &
        + Nf*(340d0/9d0 + (7744d0*zeta2)/81d0 - (32d0*zeta3)/9d0) + (112d0*zeta3)/3d0 + (12800d0*zeta4)/27d0)
        
        CASE(3)
            C_q_q_delta=&
    (-84d0 - 16d0*Nf**2/27d0 - 896d0*zeta2/9d0 + Nf*(128d0/9d0 + 256d0*zeta2/27d0) - 4096d0*zeta3/81d0)
        CASE DEFAULT
            C_q_q_delta=0d0
        END SELECT
    CASE DEFAULT
...
!!!!! n= power of alphas
!!!!! k= power of log
pure function C_g_g_delta(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp)::C_g_g_delta
    ...
    CASE(1) !! NLO
        SELECT CASE(k)
        CASE(0)
            C_g_g_delta=-3d0*zeta2
        CASE(1)
            C_g_g_delta=-(11d0-2d0/3d0*Nf)
        CASE DEFAULT
            C_g_g_delta=0d0
        END SELECT
    CASE(2) !! NNLO
        SELECT CASE(k)
        CASE(0)
            C_g_g_delta=-112d0 - 56d0*Nf**2/81d0 - 201d0*zeta2/2d0 + 154d0*zeta3 + 225d0*zeta4/4d0&
        + Nf*(548d0/27 + 5d0*zeta2 - 28d0*zeta3/3d0)
        CASE(1)
            C_g_g_delta=(-96d0 + 32d0*Nf/3d0 - 108d0*zeta3) 
        CASE(2)
            C_g_g_delta=- 72d0*zeta2
        CASE DEFAULT
            C_g_g_delta=0d0
        END SELECT
    CASE(3) !! NNNLO
        SELECT CASE(k)
        CASE(0)
            C_g_g_delta= Nf**3*(-752d0/2187d0 + (16d0*zeta3)/27d0)&
    ...
    + (15395d0*zeta4)/4d0 - 2046d0*zeta5  - (28783d0*zeta6)/16d0
      ...
      -4597d0/2d0 - (8855d0*zeta2)/2d0 - 3130d0*zeta3 + 3780d0*zeta2*zeta3 + (495d0*zeta4)/4d0 + 2160d0*zeta5)
        ...
        CASE(2)
            C_g_g_delta= &
            (-561d0 - (38d0*Nf**2)/9d0 - 3216d0*zeta2 + Nf*(311d0/3d0 + 160d0*zeta2) + 2700d0*zeta4)
        
        CASE(3)
            C_g_g_delta=- 576d0*zeta3
        CASE DEFAULT            
            C_g_g_delta=0d0
        END SELECT
    CASE DEFAULT
...
!!!!!coefficient function q<-q plus-part  (1/(1-x)_+,(Log(1-x)/(1-x))_+,(Log^2(1-x)/(1-x))_+)
pure function C_q_q_plus(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:3)::C_q_q_plus
    real(dp)::s1,s2,s3
        
    !s1 coeff 1/(1-x)
    !s2 coeff log(1-x)/(1-x)
    !s3 coeff log(1-x)^2/(1-x)
    ...
    CASE(2) !! NNLO
        SELECT CASE(k)
        CASE(0)
            s1=-3232d0/27d0+112d0*zeta3+448d0/81d0*Nf
            s2=0d0
            s3=0d0
        CASE(1)
            s1=-1072d0/9d0+352d0/9d0*zeta2+160d0/27d0*Nf
            s2=0d0
            s3=0d0
        CASE(2)
            s1=-8d0+16d0/9d0*Nf
    ...
    CASE(3) !!! N3LO
        SELECT CASE(k)
        CASE(0)
            s1=Nf**2*(-7424d0/2187d0 - (128d0*zeta3)/27d0)&
      ...
      + 616d0*zeta4 - 2304d0*zeta5
        ...
        -961208d0/243d0  + (49312d0*zeta2)/27d0 + (37760d0*zeta3)/27d0 - (34592d0*zeta4)/27d0)
            ...
            s2=(103424d0/81d0 - (14336d0*Nf)/243d0 - (3584d0*zeta3)/3d0)
            s3=0d0
        ...
        -9280d0/9d0 - (320d0*Nf**2)/81d0 + Nf*(4112d0/27d0 - (1280d0*zeta2)/27d0) + 512d0*zeta2 - (256d0*zeta3)/9d0)
            ...
            s2=(34304d0/27d0 - (5120d0*Nf)/81d0 - (10240d0*zeta2)/27d0)
            s3=0d0
            
        CASE(3)
            s1=(&
        -208d0/9d0 + (320d0*Nf)/27d0 - (64d0*Nf**2)/81d0 + (2048d0*zeta2)/27d0)
            s2=(1792d0/9d0 - (512d0*Nf)/27d0)
            s3=-2048d0/27d0
...
end function C_q_q_plus
  
!!!!!coefficient function g<-g singular-part  (1/(1-x)_+,(Log(1-x)/(1-x))_+)
pure function C_g_g_plus(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:3)::C_g_g_plus
    real(dp)::s1,s2,s3
    ...
    CASE(2) !! NNLO
        SELECT CASE(k)
        CASE(0)
            s1=-808d0/3d0 + 112d0*Nf/9d0 + 252d0*zeta3
            s2=0d0
            s3=0d0
        CASE(1)
            s1=-268d0 + 40d0*Nf/3d0 + 108d0*zeta2
            s2=0d0
            s3=0d0
        CASE(2)
            s1=66d0 - 4d0*Nf
    ...
    CASE(3) !! N3LO
        SELECT CASE(k)
        CASE(0)
            s1=Nf**2*(-1856d0/243d0 - (32d0*zeta3)/3d0) &
    ...
    -297029d0/27d0 + (8816d0*zeta2)/3d0 + 12328d0*zeta3 - 2340d0*zeta2*zeta3 + 1386d0*zeta4 - 5184d0*zeta5
            ...
            s3=0d0
        CASE(1)
            s1=(-18086d0/3d0 + (16d0*Nf**2)/9d0 + 5226d0*zeta2 + 132d0*zeta3 + &
      Nf*(4484d0/9d0 - 260d0*zeta2 + 152d0*zeta3) - 3591d0*zeta4)
            s2=(6464d0 - (896d0*Nf)/3 - 6048d0*zeta3)
            s3=0d0
        CASE(2)
            s1=(540d0 + Nf*(-52d0 - 12d0*zeta2) + 198d0*zeta2 + 1296d0*zeta3)
            s2=(6432d0 - 320d0*Nf - 2160d0*zeta2)
            s3=0d0
        CASE(3)
            s1=(242d0 - (88d0*Nf)/3d0 + (8d0*Nf**2)/9d0 + 864d0*zeta2)
            s2=0d0
            s3=-864d0
        CASE DEFAULT
            s1=0d0
...
end function C_g_g_plus


!!!!!coefficient function q<-q regular-part
pure function C_q_q_reg(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:37)::C_q_q_reg
    ...
    CASE(2) !!NNLO
        SELECT CASE(k)
        CASE(0)
            C_q_q_reg(1:13)=(/&!!!exact part
            200d0/9d0, & !Log[1-x]
      -64d0/9d0,  & !Log[1-x]^2
      0d0, 0d0, 0d0,& !log(1-x)^3,log(1-x)^4,log(1-x)^5
      0d0, 0d0, 0d0,&     !1/x, Log(x)/x, Log[x]^2/x
      -8d0 + 40d0*Nf/27d0 , & !log[x]
      -2d0 + 4d0*Nf/9d0, & !log[x]^2
      -20d0/27d0,& !Log^3(x)
      0d0, 0d0/) !Log[x]^4, Log[x]^5
            
            C_q_q_reg(14:37)= & !!!! approximate part computed by Mathematica
    ...
    CASE(3) !!N3LO
        ...
        CASE(0)
            C_q_q_reg(1:13)=(/&!!!exact part
            -37408d0/243d0 - (272d0*Nf)/729d0  &
       - (6976d0*zeta2)/81d0 - (496d0*Nf*zeta2)/81d0 + 736d0*zeta3 , & !Log[1-x]
      -7400d0/81d0 + (280d0*Nf)/27d0  + (1672d0*zeta2)/27d0,  & !Log[1-x]^2
      -2816d0/81d0 + (512d0*Nf)/243d0,& !log(1-x)^3
      0d0, 0d0, & !log(1-x)^4,log(1-x)^5
      0d0, 0d0, 0d0,&     !1/x, Log(x)/x, Log[x]^2/x
      -57152d0/27d0 - (1376d0*Nf**2)/243d0 + (27424d0*zeta2)/81d0 &
      + Nf*(64960d0/243d0 - (8096d0*zeta2)/243d0 - (1888d0*zeta3)/81d0) &
      + (31888d0*zeta3)/81d0 + (14240d0*zeta4)/81d0, &!log[x]
      -149368d0/243d0  - (592d0*Nf**2)/243d0 &
      + Nf*(57592d0/729d0 - (1024d0*zeta2)/81d0) + (3352d0*zeta2)/27d0 &
      + (2872d0*zeta3)/27d0, &  !log[x]^2
      -26932d0/243d0 + (8704d0*Nf)/729d0 - (80d0*Nf**2)/243d0 &
      + (6392d0*zeta2)/243d0, & !Log^3(x)
      -704d0/81d0  + (208d0*Nf)/243d0, & !Log[x]^4,
      -122d0/405d0/) !Log[x]^5
      ...
      CASE(1)
        C_q_q_reg(1:13)=(/&!!!exact part
         - (183152d0)/81d0 + (26336d0*Nf)/243d0 + (8704d0*zeta2)/27d0 &
       + (3584d0*zeta3)/3d0, & !Log[1-x]
       - (6656d0)/27d0 + (256d0*Nf)/27d0,  & !Log[1-x]^2
       + (1024d0)/27d0 ,& !log(1-x)^3
      0d0, 0d0, & !log(1-x)^4,log(1-x)^5
      0d0, 0d0, 0d0,&     !1/x, Log(x)/x, Log[x]^2/x
      (-144304d0/81d0 - (512d0*Nf**2)/81d0 + Nf*(58288d0/243d0 &
      - (1024d0*zeta2)/27d0) + (3200d0*zeta2)/9d0 + (928d0*zeta3)/9d0), &!log[x]
     + (-4672d0/9d0 + (1280d0*Nf)/27d0 - (32d0*Nf**2)/27d0 + (1024d0*zeta2)/9d0), &  !log[x]^2
      (-3704d0/81d0 + (112d0*Nf)/27d0), & !Log^3(x)
       - 2d0 , & !Log[x]^4,
      0d0/) !Log[x]^5
      ...
      CASE(2)
        C_q_q_reg(1:13)=(/&!!!exact part
       - (45056d0)/27d0 + (8192d0*Nf)/81d0 + (10240d0*zeta2)/27d0, & !Log[1-x]
       (4096d0)/27d0 ,  & !Log[1-x]^2
      0d0,& !log(1-x)^3
      0d0, 0d0, & !log(1-x)^4,log(1-x)^5
      0d0, 0d0, 0d0,&     !1/x, Log(x)/x, Log[x]^2/x
      + (-13144d0/27d0 + (3520d0*Nf)/81d0 - (32d0*Nf**2)/27d0 &
      + (2560d0*zeta2)/27d0) , &!log[x]
      (-632d0/9d0 + (16d0*Nf)/3d0), &  !log[x]^2
       - (320d0)/81d0, & !Log^3(x)
      0d0, & !Log[x]^4,
      0d0/) !Log[x]^5
    ...
    CASE(3)
        C_q_q_reg(1:13)=(/&!!!exact part
      - (7424d0)/27d0 + (512d0*Nf)/27d0 , & !Log[1-x]
    (2048d0)/27d0 ,  & !Log[1-x]^2
      0d0,& !log(1-x)^3
      0d0, 0d0, & !log(1-x)^4,log(1-x)^5
      0d0, 0d0, 0d0,&     !1/x, Log(x)/x, Log[x]^2/x
       (-928d0/27d0 + (64d0*Nf)/27d0) , &!log[x]
       - (128d0)/81d0 , &  !log[x]^2
      0d0, & !Log^3(x)
      0d0, & !Log[x]^4,
      0d0/) !Log[x]^5
	...
	0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0/)
    END SELECT
end function C_q_q_reg
 
!!!!!coefficient function q<-g regular-part 
pure function C_q_g_reg(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:37)::C_q_g_reg
    ...
    CASE(2) !!NNLO
        SELECT CASE(k)
        CASE(0)
            C_q_g_reg(1:13)= (/&!!!Exact part
      -5d0/3d0, 0d0, 5d0/9d0,& !log(1-x),log(1-x)^2,log(1-x)^3
      0d0, 0d0,& !log(1-x)^4,log(1-x)^5
      172d0/9d0 - 8d0*zeta2, 0d0, 0d0, &     !1/x, Log(x)/x, Log[x]^2/x
      34d0/3d0, -7d0/6d0, 7d0/9d0, 0d0, 0d0/) !Log(x),Log^2(x),Log^3(x),Log^4(x),Log^5(x)
      
        C_q_g_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    4921.711750117209d0, -500.29279582171137d0, -2549.79132306537d0, 272.6427824324629d0/)
        ...
        CASE(1)
            C_q_g_reg(1:13)= (/&!!!Exact part
       0d0, 10d0/3d0, 0d0,& !log(1-x),log(1-x)^2,log(1-x)^3
      0d0, 0d0,& !log(1-x)^4,log(1-x)^5
       - 52d0/3d0, 0d0, 0d0, &     !1/x, Log(x)/x, Log[x]^2/x
      - 22d0/3d0, 14d0/3d0, 0d0, 0d0, 0d0/) !Log(x),Log^2(x),Log^3(x),Log^4(x),Log^5(x)
            C_q_g_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    /)
        ...
        CASE(2)
            C_q_g_reg(1:13)= (/&!!!Exact part
      + 26d0/3d0, 0d0, 0d0,& !log(1-x),log(1-x)^2,log(1-x)^3
      0d0, 0d0,& !log(1-x)^4,log(1-x)^5
       4d0, 0d0, 0d0, &     !1/x, Log(x)/x, Log[x]^2/x
       14d0/3d0, 0d0,0d0, 0d0, 0d0/) !Log(x),Log^2(x),Log^3(x),Log^4(x),Log^5(x)
            C_q_g_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    CASE(3)
        ...
        CASE(0)
            C_q_g_reg(1:13)=(/&!!!exact part
     -22412d0/243d0 - (422d0*zeta2)/27d0 - 58d0*zeta3 &
     + Nf*(5378d0/729d0 + (20d0*zeta2)/9d0 + (28d0*zeta3)/3d0) + (730d0*zeta4)/9d0, & !log[1-x]
     8821d0/162d0 - (166d0*zeta2)/9d0 + Nf*(-173d0/243d0 + (4d0*zeta2)/3d0) &
     - (1310d0*zeta3)/27d0, &  !log[1-x]^2
     1870d0/81d0 - (200d0*Nf)/243d0 - (500d0*zeta2)/81d0, & !log[1-x]^3
     85d0/36d0 - (25d0*Nf)/162d0, & !log[1-x]^4
     -25d0/27d0, & !log[1-x]^5
     -480178d0/81d0 + (35192d0*zeta2)/27d0 &
     + Nf*(66130d0/2187d0 - (304d0*zeta2)/81d0 - (272d0*zeta3)/27d0) + (11912d0*zeta3)/9d0 &
     + (2968d0*zeta4)/3d0, & !1/x
     -17152d0/27d0 + 208d0*zeta2 + 96d0*zeta3, & !Log(x)/x
     0d0,&     !Log[x]^2/x
     -567080d0/243d0 + (12262d0*zeta2)/27d0 + (164d0*zeta3)/9d0 &
     + Nf*(139178d0/729d0 + 8d0*zeta2 + (32d0*zeta3)/3d0) - (39473d0*zeta4)/54d0, & !log[x]
     22754d0/81d0 + (860d0*zeta2)/27d0 &
     + Nf*(14206d0/243d0 + (16d0*zeta2)/3d0) - (9748d0*zeta3)/27d0, & !log[x]^2
     -13136d0/81d0 + (3466d0*Nf)/243d0 - (523d0*zeta2)/81d0, & !Log^3(x)
     823d0/162d0 + (103d0*Nf)/81d0, & !Log[x]^4,
     ...
     -2524447.0539041213d0, 1495477.2954864914d0, -45392.404861584015d0, 738745.8988557612d0/)
        ...
        CASE(1)
            C_q_g_reg(1:13)=(/&!!!exact part
     (2708d0/9d0 - 42d0*zeta2 + Nf*(-74d0/9d0 + 4d0*zeta2) &
     - (1636d0*zeta3)/9d0) , & !log[1-x]
     (1130d0/9d0 - (100d0*Nf)/27d0 - (40d0*zeta2)/3d0), &  !log[1-x]^2
     (10d0 - (20d0*Nf)/27d0), & !log[1-x]^3
     - (230d0)/27d0, & !log[1-x]^4
     0d0, & !log[1-x]^5
     (9188d0/3d0 &
     - (6560d0*zeta2)/9d0 + Nf*(-4208d0/243d0 + (272d0*zeta2)/27d0) - (1712d0*zeta3)/3d0), & !1/x
     (1136d0/3d0 - 96d0*zeta2), & !Log(x)/x
     0d0,&     !Log[x]^2/x
     (5416d0/9d0 + (1642d0*zeta2)/9d0 &
     + Nf*(4120d0/27d0 + (32d0*zeta2)/3d0) - (7816d0*zeta3)/9d0), & !log[x]
     (-10039d0/18d0 + (1567d0*Nf)/27d0 - (235d0*zeta2)/3d0) , & !log[x]^2
     (535d0/27d0 + (190d0*Nf)/27d0), & !Log^3(x)
     (-335d0/27d0 + (10d0*Nf)/9d0), & !Log[x]^4,
     0d0/) !Log[x]^5
         C_q_g_reg(14:37)= (/& !!!! approximate part computed by Mathematica
     ...
     /)
        ...
        CASE(2)
            C_q_g_reg(1:13)=(/&!!!exact part
     (910d0/3d0 - (260d0*Nf)/27d0), & !log[1-x]
     (35d0/3d0 - (10d0*Nf)/9d0) , &  !log[1-x]^2
      - (260d0)/9d0 , & !log[1-x]^3
     80d0, & !log[1-x]^4
     0d0, & !log[1-x]^5
     (-1790d0/3d0 + (152d0*Nf)/81d0 + (128d0*zeta2)/3d0), & !1/x
      - 104d0 , & !Log(x)/x
     0d0,&     !Log[x]^2/x
     (-3718d0/9d0 + (1360d0*Nf)/27d0 - (1592d0*zeta2)/9d0), & !log[x]
     (40d0/3d0 + (80d0*Nf)/9d0) , & !log[x]^2
     (-536d0/27d0 + (16d0*Nf)/9d0), & !Log^3(x)
     0d0, & !Log[x]^4,
     0d0/) !Log[x]^5
        C_q_g_reg(14:37)= & !!!! approximate part computed by Mathematica
     ...
     /)
        ...
        CASE(3)
            C_q_g_reg(1:13)=(/&!!!exact part
     (238d0/9d0 - (68d0*Nf)/27d0), & !log[1-x]
      - (1064d0)/27d0 , &  !log[1-x]^2
     0d0, & !log[1-x]^3
     0d0, & !log[1-x]^4
     0d0, & !log[1-x]^5
     (272d0/3d0 - (136d0*Nf)/81d0), & !1/x
    16d0, & !Log(x)/x
     0d0,&     !Log[x]^2/x
      (110d0/9d0 + (52d0*Nf)/27d0), & !log[x]
      (-268d0/27d0 + (8d0*Nf)/9d0) , & !log[x]^2
     0d0, & !Log^3(x)
     0d0, & !Log[x]^4,
     0d0/) !Log[x]^5
...
end function C_q_g_reg
  
!!!!!coefficient function g<-q regular-part 
pure function C_g_q_reg(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:37)::C_g_q_reg
	...
	CASE(1)
            C_g_q_reg=(/&
      0d0, 0d0, 0d0, 0d0, 0d0,& !log(1-x),log(1-x)^2,log(1-x)^3,log(1-x)^4,log(1-x)^5
      -16d0/3d0, 0d0, 0d0,&     !1/x, Log(x)/x,Log[x]^2/x
      0d0, 0d0, 0d0, 0d0, 0d0,& !Log(x),Log^2(x),Log^3(x),Log^4(x),Log^5(x)
      16d0/3d0,-8d0/3d0, 0d0,& !1,x,x(1-x)
	0d0, 0d0, 0d0,& !...
	0d0, 0d0, 0d0,& !...
	0d0, 0d0, 0d0,& !...
	0d0, 0d0, 0d0,& !...
    ...
    CASE(2) !!NNLO
        SELECT CASE(k)
        CASE(0)
            C_g_q_reg(1:13)=(/ &!!Exact part
      -184d0/9d0 + 32d0*Nf/27d0, & !log(1-x),
	  -44d0/9d0 + 8d0*Nf/9d0, &!log(1-x)^2
	  -40d0/27d0,& !log(1-x)^3
	  0d0, 0d0, & !log[1-x]^4, Log[1-x]^5
      -12640d0/27d0 + 896d0*Nf/81d0 + 352d0*zeta2/3d0 + 192d0*zeta3, & ! 1/x
	  0d0,&     !Log(x)/x
	  0d0, & ! log[x]^2/x
      -200d0/3d0,& ! log[x]
      112d0/9d0 , & !log[x]^2
      -112d0/27d0,& !Log^3(x)
      0d0, 0d0/)  !log^4[x], Log^5[x]
      C_g_q_reg(14:37) = &!!!! approximate part computed by Mathematica
     ...
     5670.649765218949d0, -286.7508347433035d0, -3920.619711189592d0, 1269.1159656155107d0/)
        CASE(1)
            C_g_q_reg(1:13)=(/ &!!Exact part
      (-208d0/3d0 + 32d0*Nf/9d0), & !log(1-x),
	   - 80d0, &!log(1-x)^2
	  0d0,& !log(1-x)^3
	  0d0, 0d0, & !log[1-x]^4, Log[1-x]^5
        (-16d0 + 320d0*Nf/27d0 + 16d0*zeta2), & ! 1/x
	  0d0,&     !Log(x)/x
	  0d0, & ! log[x]^2/x
        448d0/9d0,& ! log[x]
       - 224d0/9d0, & !log[x]^2
    ...
    CASE(3) !! N3LO
        ...
        CASE(0)
            C_g_q_reg(1:13)=(/ &!!Exact part
      -2606752d0/729d0 - (224d0*Nf**2)/27d0 &
      + (19240d0*zeta2)/81d0 + (92416d0*zeta3)/81d0 &
      ...
      - (352d0*Nf**2)/81d0 + Nf*(102664d0/729d0 - (296d0*zeta2)/27d0) + (7124d0*zeta2)/81d0 &
        + (4240d0*zeta3)/81d0,& ! Log^2[1-x]
      -56852d0/243d0 + (20672d0*Nf)/729d0 - (80d0*Nf**2)/81d0 &
       + (4600d0*zeta2)/243d0, & ! Log^3[1-x]
      -7430d0/243d0 + (700d0*Nf)/243d0, &! Log[1-x]^4
      -200d0/81d0, & !Log[1-x]^5
      5955362d0/81d0 - (369280d0*zeta2)/27d0 + Nf**2*(-7424d0/729d0 &
      ...
      + (32d0*zeta4)/9d0) - (378688d0*zeta5)/27d0, & !! 1/x
      302336d0/27d0 - (30016d0*zeta2)/9d0 &
       - 960d0*zeta3 + Nf*(-75424d0/2187d0 &
      - (1024d0*zeta2)/243d0 + (2432d0*zeta3)/27d0) - 1568d0*zeta4, & !! log[x]/x
        + 768d0*zeta3, & !! log^2[x]/x
      51537256d0/3645d0 - (42568d0*zeta2)/27d0 &
        - (357760d0*zeta3)/81d0 &
      + Nf*(-4044016d0/10935d0 + (8960d0*zeta2)/243d0 + (7168d0*zeta3)/81d0) &
        + (108248d0*zeta4)/27d0, & ! log[x]
      -850448d0/243d0 &
      - (25480d0*zeta2)/81d0 + Nf*(-89776d0/729d0 + (896d0*zeta2)/81d0) + 2368d0*zeta3, &! log[x]^2
      63392d0/81d0 &
      - (28768d0*Nf)/729d0 + (704d0*zeta2)/9d0, &! log[x]^3
      -4760d0/81d0  - (688d0*Nf)/243d0, & ! Log[x]^4
    ...
    779457.2094747808d0, -1090149.6774086975d0, 941888.7159622173d0, -994158.7838174688d0/)
        CASE(1)
            C_g_q_reg(1:13)=(/ &!!Exact part
      (-186472d0/81d0 - (896d0*Nf**2)/81d0 + Nf*(87712d0/243d0 &
      - (832d0*zeta2)/27d0) + (7328d0*zeta2)/9d0 - (34208d0*zeta3)/27d0) , & !log[1-x]
      (-6388d0/9d0 + (2272d0*Nf)/27d0 - (80d0*Nf**2)/27d0 + 80d0*zeta2),& ! Log^2[1-x]
      (-2216d0/27d0 + (176d0*Nf)/81d0), & ! Log^3[1-x]
      (80d0)/27d0 , &! Log[1-x]^4
      0d0, & !Log[1-x]^5
       (-459260d0/9d0 - (704d0*Nf**2)/81d0 + (301208d0*zeta2)/27d0 &
      + Nf*(65728d0/81d0 - (13552d0*zeta2)/81d0 - (512d0*zeta3)/9d0) + (140576d0*zeta3)/9d0 &
      + (134924d0*zeta4)/27d0), & !! 1/x
      (-25280d0/3d0 + 2112d0*zeta2 + &
      Nf*(13376d0/243d0 + (128d0*zeta2)/27d0) + 2688d0*zeta3), & !! log[x]/x
      0d0, & !! log^2[x]/x
      (-538744d0/81d0 - (19376d0*zeta2)/9d0 + Nf*(-74864d0/243d0 + (1792d0*zeta2)/27d0) &
      + (129536d0*zeta3)/27d0) , & ! log[x]
     (55744d0/27d0 &
      - (7136d0*Nf)/81d0 + (19120d0*zeta2)/27d0) , &! log[x]^2
        (-2576d0/9d0 - (928d0*Nf)/81d0), &! log[x]^3
      (5360d0/81d0 - (160d0*Nf)/27d0), & ! Log[x]^4
      0d0/)  ! Log^5[x]
            C_g_q_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    /) 
        CASE(2)
            C_g_q_reg(1:13)=(/ &!!Exact part
      (13096d0/9d0 - (1696d0*Nf)/27d0 - (32d0*Nf**2)/27d0 - (13520d0*zeta2)/27d0), & !log[1-x]
      (4360d0/9d0 - (752d0*Nf)/27d0),& ! Log^2[1-x]
      (2080d0)/27d0, & ! Log^3[1-x]
      0d0, &! Log[1-x]^4
      0d0, & !Log[1-x]^5
      (140896d0/27d0 - (320d0*Nf**2)/81d0 - (9016d0*zeta2)/9d0 &
      + Nf*(62248d0/243d0 + 32d0*zeta2) - (41152d0*zeta3)/27d0), & !! 1/x
      (-96d0 + (9056d0*Nf)/81d0 + 96d0*zeta2), & !! log[x]/x
      0d0, & !! log^2[x]/x
      (37568d0/27d0 - 64d0*Nf + (31072d0*zeta2)/27d0), & ! log[x]
      (-3584d0/9d0 - (128d0*Nf)/27d0), &! log[x]^2
      (8576d0/81d0 - (256d0*Nf)/27d0), &! log[x]^3
      0d0, & ! Log[x]^4
      0d0/)  ! Log^5[x]
            C_g_q_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    /)
        CASE(3)
            C_g_q_reg(1:13)=(/ &!!Exact part
      (1904d0/27d0 - (544d0*Nf)/81d0) , & !log[1-x]
       - (8512d0)/81d0,& ! Log^2[1-x]
      0d0, & ! Log^3[1-x]
      -0d0, &! Log[1-x]^4
      0d0, & !Log[1-x]^5
      (-5128d0/3d0 + (8432d0*Nf)/243d0 &
      + (2816d0*zeta2)/9d0) , & !! 1/x
      (-2048d0/3d0 + (1088d0*Nf)/81d0), & !! log[x]/x
      -64d0, & !! log^2[x]/x
        (-224d0 + (320d0*Nf)/81d0) , & ! log[x]
       (4288d0/81d0 - (128d0*Nf)/27d0), &! log[x]^2
      0d0, &! log[x]^3
      0d0, & ! Log[x]^4
      0d0/)  ! Log^5[x]
            C_g_q_reg(14:37) = &!!!! approximate part computed by Mathematica
...
end function C_g_q_reg
  
!!!!!coefficient function g<-g regular-part 
pure function C_g_g_reg(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:37)::C_g_g_reg
    
    SELECT CASE(n)
    !! LO=0
    CASE(1) !!NLO
        SELECT CASE(k)
        CASE(1)!!! only log part
            C_g_g_reg=(/&
      0d0, 0d0, 0d0, 0d0, 0d0,& !log(1-x),log(1-x)^2,log(1-x)^3,log(1-x)^4,log(1-x)^5
    ...
    CASE(2) !!NNLO
        SELECT CASE(k)
        CASE(0)
            C_g_g_reg(1:13)=(/ &!!Exact part
      6d0  - 2d0*Nf, & !log(1-x),
	  -36d0, &!log(1-x)^2
	  0d0, 0d0, 0d0, & !log[1-x]^3, log[1-x]^4, Log[1-x]^5
      -3160d0/3d0 + (226d0*Nf)/9d0 + 264d0*zeta2 + &
        + 432d0*zeta3, & ! 1/x
	  0d0,&     !Log(x)/x
	  0d0, & ! log[x]^2/x
      -293d0 + (74d0*Nf)/3d0,& ! log[x]
       3d0 + 6d0*Nf, & !log[x]^2
   ...
   (/ -1.333333333333333d0, -39.1111111111111d0, 37.77777777777778d0, 0d0, &
    ...
    /)
	  ...
	  0d0, &!log(1-x)^2
	  0d0, 0d0, 0d0, & !log[1-x]^3, log[1-x]^4, Log[1-x]^5
      (-198d0 - (4d0*Nf)/9d0), & ! 1/x
	  -72d0,&     !Log(x)/x
	  0d0, & ! log[x]^2/x
      (-72d0 + (16d0*Nf)/3d0),& ! log[x]
       0d0, & !log[x]^2
      0d0,& !Log^3(x)
      0d0, 0d0/)  !log^4[x], Log^5[x]
            C_g_g_reg(14:37) = &
    ...
    CASE(3) !!N3LO
        ...
        CASE(0)
            C_g_g_reg(1:13)=(/ &!!Exact part
     -14020d0/3d0 + (56d0*Nf**2)/9d0 + Nf*(592d0/9d0 - (182d0*zeta2)/3d0) &
     + 1518d0*zeta2 + 2736d0*zeta3, & !log(1-x),
     -1432d0 + (8d0*Nf**2)/9d0 &
     + 468d0*zeta2 + Nf*(164d0/3d0 + 6d0*zeta2),& !log(1-x)^2
     -176d0 + (32d0*Nf)/3d0, & !log[1-x]^3,
     0d0, 0d0, & !log[1-x]^4, Log[1-x]^5
     1572769d0/9d0 - (103448d0*zeta2)/3d0 + Nf**2*(-54584d0/2187d0 - (800d0*zeta3)/27d0) &
     - 16128d0*zeta3 + 2160d0*zeta2*zeta3 - 41184d0*zeta4 + Nf*(743104d0/729d0 &
     - (17254d0*zeta2)/81d0 - (23296d0*zeta3)/81d0 + (25544d0*zeta4)/81d0) - 33264d0*zeta5, & ! 1/x
     75584d0/3d0 - 7504d0*zeta2 - 1584d0*zeta3 + Nf*(-18640d0/243d0 - (256d0*zeta2)/27d0 + 224d0*zeta3) - 3528d0*zeta4, & ! log[x]/x
      + 1728d0*zeta3, & ! log[x]^2/x
     164484d0/5d0 + 3343d0*zeta2 &
    + Nf**2*(-30316d0/729d0 - (1888d0*zeta2)/81d0 - (512d0*zeta3)/27d0) &
     - 3888d0*zeta3 &
     + Nf*(-2047364d0/1215d0 - (766d0*zeta2)/3d0 - (23968d0*zeta3)/27d0 - (19546d0*zeta4)/27d0) &
     + 11448d0*zeta4, & ! Log[x]
     -6667d0 &
     + Nf**2*(-3724d0/243d0 - (64d0*zeta2)/27d0) + 927d0*zeta2 + Nf*(-38672d0/81d0 - (6592d0*zeta2)/27d0 &
     - (10432d0*zeta3)/27d0) + 6336d0*zeta3, & ! Log^2[x]
     7034d0/3d0  - (296d0*Nf**2)/243d0  + Nf*(-17968d0/81d0 - (1844d0*zeta2)/81d0) + 468d0*zeta2, & ! Log^3[x]
     -62d0 - (1568d0*Nf)/81d0 + (8d0*Nf**2)/81d0, & ! Log^4[x]
     144d0/5d0 - (512d0*Nf)/135d0 /)  ! Log^5[x]
            C_g_g_reg(14:37) = &!!!! approximate part computed by Mathematica
      ...
      -1379746.8890004286d0, -6318383.538237535d0, 13323182.55528821d0, -9914427.904937435d0/)
        ...
        CASE(1)
            C_g_g_reg(1:13)=(/ &!!Exact part
     (-12974d0 + (1922d0*Nf)/3d0 + (4d0*Nf**2)/3d0 + 432d0*zeta2 + 6048d0*zeta3), & !log(1-x),
     (-468d0 + 48d0*Nf),& !log(1-x)^2
    432d0 , & !log[1-x]^3,
     0d0, 0d0, & !log[1-x]^4, Log[1-x]^5
     (-354278d0/3d0 &
     - (1652d0*Nf**2)/81d0 + 25686d0*zeta2 + Nf*(15230d0/9d0 - 360d0*zeta2 - (6496d0*zeta3)/27d0) &
     + 38544d0*zeta3 + 8937d0*zeta4) , & ! 1/x
     (-18960d0 + 4752d0*zeta2 + Nf*(3416d0/27d0+ (32d0*zeta2)/3d0) + 6048d0*zeta3), & ! log[x]/x
     0d0, & ! log[x]^2/x
     (-13945d0 + Nf**2*(-364d0/9d0 - (128d0*zeta2)/9d0) + 324d0*zeta2 &
     + Nf*(-7568d0/9d0 - (5788d0*zeta2)/9d0 - (5824d0*zeta3)/9d0) + 12096d0*zeta3), & ! Log[x]
     (6483d0 - (404d0*Nf**2)/27d0 &
     + Nf*(-528d0 - (1444d0*zeta2)/9d0) + 2808d0*zeta2), & ! Log^2[x]
     (-288d0 - (280d0*Nf)/3d0 + (16d0*Nf**2)/27d0) , & ! Log^3[x]
     (180d0 - (640d0*Nf)/27d0) , & ! Log^4[x]
     0d0 /)  ! Log^5[x]
            C_g_g_reg(14:37) = &!!!! approximate part computed by Mathematica
      ...
      /)
        ...
        CASE(2)
            C_g_g_reg(1:13)=(/ &!!Exact part
     (-6432d0 + 320d0*Nf + 2160d0*zeta2), & !log(1-x),
     1728d0,& !log(1-x)^2
     0d0, & !log[1-x]^3,
     0d0, 0d0, & !log[1-x]^4, Log[1-x]^5
     (12532d0 - (640d0*Nf**2)/81d0 - 2178d0*zeta2 + Nf*(5924d0/9d0 &
     + 20d0*zeta2) - 3456d0*zeta3), & ! 1/x
     ((2288d0*Nf)/9d0 + 216d0*zeta2) , & ! log[x]/x
     0d0, & ! log[x]^2/x
     (3984d0 - (512d0*Nf**2)/27d0 + Nf*(-2080d0/9d0 - (2128d0*zeta2)/9d0) &
     + 3672d0*zeta2), & ! Log[x]
     (-504d0 - (656d0*Nf)/9d0), & ! Log^2[x]
     (288d0 - (1024d0*Nf)/27d0), & ! Log^3[x]
     0d0, & ! Log^4[x]
     0d0 /)  ! Log^5[x]
            C_g_g_reg(14:37) = &!!!! approximate part computed by Mathematica
     ...
     0d0, & !log[1-x]^3,
     0d0, 0d0, & !log[1-x]^4, Log[1-x]^5
     (-4270d0 + 88d0*Nf + (8d0*Nf**2)/81d0 &
     + 864d0*zeta2), & ! 1/x
     (-1584d0 + (256d0*Nf)/9d0), & ! log[x]/x
     -144d0, & ! log[x]^2/x
     (-432d0 + (176d0*Nf)/9d0 - (32d0*Nf**2)/27d0), & ! Log[x]
     (144d0 - (512d0*Nf)/27d0), & ! Log^2[x]
     0d0, & ! Log^3[x]
     0d0, & ! Log^4[x]
     0d0 /)  ! Log^5[x]
            C_g_g_reg(14:37) = &!!!! approximate part computed by Mathematica
...
end function C_g_g_reg

!!!!!coefficient function q<-qb regular-part
pure function C_q_qb_reg(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:37)::C_q_qb_reg
    ...
    CASE(2) !!NLO
        SELECT CASE(k)
        CASE(0)!!! only log part
            C_q_qb_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      0d0, 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
      -4d0/3d0 ,& ! log[x]
       0d0, & !log[x]^2
      4d0/27d0,& !Log^3(x)
      0d0, 0d0/)  !log^4[x], Log^5[x]
            C_q_qb_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    CASE(3) !!N3LO
      ...
      0d0, 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
      54976d0/243d0 &
      - (3344d0*zeta2)/81d0 + Nf*(704d0/729d0 - (736d0*zeta2)/243d0 &
      - (32d0*zeta3)/81d0) - (6448d0*zeta3)/81d0 + (1484d0*zeta4)/27d0, & !log[x]
      18620d0/243d0 + Nf*(-1544d0/729d0 - (64d0*zeta2)/81d0) + (2672d0*zeta2)/81d0 &
      - (7432d0*zeta3)/81d0, & !log^2[x]
      6544d0/243d0 - (688d0*Nf)/729d0 - (2240d0*zeta2)/243d0, & !log^3[x]
      28d0/81d0 - (32d0*Nf)/243d0, & !log^4[x]
      38d0/135d0/)  !Log^5[x]
            C_q_qb_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    -112765.1374329637d0, -59211.74145105641d0, 180432.8596727076d0, -118492.80482198505d0/)
      ...
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      0d0, 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
      (5744d0/27d0 + Nf*(-512d0/81d0 - (32d0*zeta2)/27d0) &
      + (1216d0*zeta2)/27d0 - (1376d0*zeta3)/27d0), & !log[x]
      (3488d0/27d0 - (304d0*Nf)/81d0 &
      - (1336d0*zeta2)/27d0), & !log^2[x]
      (184d0/81d0 - (16d0*Nf)/27d0), & !log^3[x]
        (122d0)/81d0 , & !log^4[x]
      0d0/)  !Log^5[x]
            C_q_qb_reg(14:37) = &!!!! approximate part computed by Mathematica
    ...
    /) 
        ...
        CASE(2)!!! only log part
            C_q_qb_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      0d0, 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
      (848d0/27d0 - (32d0*Nf)/27d0 - (128d0*zeta2)/9d0), & !log[x]
      (296d0/27d0 - (16d0*Nf)/27d0), & !log^2[x]
      (64d0)/81d0, & !log^3[x]
      0d0, & !log^4[x]
      0d0/)  !Log^5[x]
            C_q_qb_reg(14:37) = &!!!! approximate part computed by Mathematica
...
end function C_q_qb_reg
  
!!!!!coefficient function q<-qp regular-part 
pure function C_q_qp_reg(n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k
    real(dp),dimension(1:37)::C_q_qp_reg
  ...
  !------The kernels are calculated in mathematica
            C_q_qp_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      688d0/81d0  - 32d0*zeta2/9d0, 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
      8d0/3d0, &! log[x]
      -2d0/3d0, & !log[x]^2
      4d0/9d0,& !Log^3(x)
      0d0, 0d0/)  !log^4[x], Log^5[x]
            C_q_qp_reg(14:37) = &!!!! approximate part computed by Mathematica
  ...
  !------The kernels are calculated in mathematica
            C_q_qp_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
       - 208d0/27d0 , 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
       - 8d0/3d0, &! log[x]
       8d0/3d0, & !log[x]^2
      0d0,& !Log^3(x)
      0d0, 0d0/)  !log^4[x], Log^5[x]
  ...
  !------The kernels are calculated in mathematica
            C_q_qp_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
       16d0/9d0, 0d0, 0d0, & ! 1/x, Log(x)/x, log[x]^2/x
       8d0/3d0, &! log[x]
      0d0, & !log[x]^2
      0d0,& !Log^3(x)
      0d0, 0d0/)  !log^4[x], Log^5[x]
    ...
    CASE(3) !!N3LO
        ...
        CASE(0)
            C_q_qp_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      -1863800d0/729d0  + (134432d0*zeta2)/243d0 &
      + Nf*(33856d0/2187d0 - (64d0*zeta2)/27d0 - (128d0*zeta3)/27d0) + (47072d0*zeta3)/81d0 &
      + (11872d0*zeta4)/27d0, & ! 1/x,
      -68608d0/243d0 + (832d0*zeta2)/9d0 + (128d0*zeta3)/3d0, & ! Log(x)/x
      0d0, & ! Log^2(x)/x
      -150656d0/243d0 + (6304d0*zeta2)/27d0 + Nf*(-15760d0/729d0 &
      - (320d0*zeta2)/81d0 - (64d0*zeta3)/27d0) + 80d0*zeta3 - (9808d0*zeta4)/27d0, &  !log[x]
      22244d0/81d0 + Nf*(-1616d0/243d0 - (64d0*zeta2)/27d0) &
      - (272d0*zeta2)/27d0 - (3856d0*zeta3)/27d0, & !log^2[x]
      -3044d0/81d0  - (392d0*Nf)/243d0 &
      - (512d0*zeta2)/81d0, & !log^3[x]
      580d0/81d0 - (20d0*Nf)/81d0, & !log^4[x]
      -20d0/27d0/)   !log^5[x]
            C_q_qp_reg(14:37) = &!!!! approximate part computed by Mathematica
      ...
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      (106112d0/81d0 &
      - (25664d0*zeta2)/81d0 + Nf*(-2176d0/243d0 + (128d0*zeta2)/27d0) - (6848d0*zeta3)/27d0),& ! 1/x,
      (4544d0/27d0 - (128d0*zeta2)/3d0), & ! Log(x)/x
      0d0, & ! Log^2(x)/x
      (16352d0/27d0 + Nf*(-1472d0/81d0 &
      - (64d0*zeta2)/9d0) + (256d0*zeta2)/3d0 - (1184d0*zeta3)/3d0), &  !log[x]
      (-908d0/9d0 &
      - (208d0*Nf)/27d0 - (352d0*zeta2)/9d0) , & !log^2[x]
      (920d0/27d0 - (32d0*Nf)/27d0), & !log^3[x]
       - (40d0)/9d0, & !log^4[x]
      0d0/)   !log^5[x]
            C_q_qp_reg(14:37) = &!!!! approximate part computed by Mathematica
      ...
      /)
        ...
        CASE(2)
            C_q_qp_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      (-6808d0/27d0 + (32d0*Nf)/27d0 + (512d0*zeta2)/27d0), & ! 1/x,
      - (416d0)/9d0, & ! Log(x)/x
      0d0, & ! Log^2(x)/x
      (-152d0/9d0 - (208d0*Nf)/27d0 - (704d0*zeta2)/9d0) , &  !log[x]
      (344d0/9d0 - (16d0*Nf)/9d0), & !log^2[x]
       - (160d0)/27d0, & !log^3[x]
      0d0, & !log^4[x]
      0d0/)   !log^5[x]
            C_q_qp_reg(14:37) = &!!!! approximate part computed by Mathematica
      ...
      /)
        ...
        CASE(3)
            C_q_qp_reg(1:13)=(/ &!!Exact part
      0d0, 0d0, 0d0, 0d0, 0d0, & !log(1-x), log(1-x)^2, log[1-x]^3, log[1-x]^4, Log[1-x]^5
      (352d0/9d0 - (64d0*Nf)/81d0) , & ! 1/x,
       (64d0)/9d0, & ! Log(x)/x
      0d0, & ! Log^2(x)/x
      (176d0/9d0 - (32d0*Nf)/27d0), &  !log[x]
       - (80d0)/27d0, & !log^2[x]
      0d0, & !log^3[x]
      0d0, & !log^4[x]
      0d0/)   !log^5[x]
	...
	0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0/)
    END SELECT
end function C_q_qp_reg

function CoefREG_q_q(x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    integer,intent(in)::n,k
    real(dp)::CoefREG_q_q
    
    CoefREG_q_q=sum(parametrizationString(x)*C_q_q_reg(n,k,Nf))
end function CoefREG_q_q

function CoefREG_q_g(x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    integer,intent(in)::n,k
    real(dp)::CoefREG_q_g
    
    CoefREG_q_g=sum(parametrizationString(x)*C_q_g_reg(n,k,Nf))
end function CoefREG_q_g

function CoefREG_g_q(x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    integer,intent(in)::n,k
    real(dp)::CoefREG_g_q
    
    CoefREG_g_q=sum(parametrizationString(x)*C_g_q_reg(n,k,Nf))
end function CoefREG_g_q

function CoefREG_g_g(x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    integer,intent(in)::n,k
    real(dp)::CoefREG_g_g
    
    CoefREG_g_g=sum(parametrizationString(x)*C_g_g_reg(n,k,Nf))
end function CoefREG_g_g

function CoefREG_q_qb(x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    integer,intent(in)::n,k
    real(dp)::CoefREG_q_qb
    
    CoefREG_q_qb=sum(parametrizationString(x)*C_q_qb_reg(n,k,Nf))
end function CoefREG_q_qb

function CoefREG_q_qp(x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    integer,intent(in)::n,k
    real(dp)::CoefREG_q_qp
    
    CoefREG_q_qp=sum(parametrizationString(x)*C_q_qp_reg(n,k,Nf))
end function CoefREG_q_qp

!!!! this function is universal interface flav: 1=qq,2=qg,3=gq,4=gg,5=qqb,6=qqp
function CoefREG(flav,x,n,k,Nf)
    real(dp),intent(in)::Nf,x
    ...
    SELECT CASE(flav)
    CASE(1)
        CoefREG=CoefREG_q_q(x,n,k,Nf)
    CASE(2)
        CoefREG=CoefREG_q_g(x,n,k,Nf)
    CASE(3)
        CoefREG=CoefREG_g_q(x,n,k,Nf)
    CASE(4)
        CoefREG=CoefREG_g_g(x,n,k,Nf)
    CASE(5)
    ...
    CASE DEFAULT
        ...
        stop
    END SELECT
end function CoefREG

!!!! this function is universal interface flav: 1=qq,2=qg,3=gq,4=gg,5=qqb,6=qqp
function CoefSING(flav,n,k,Nf)
    real(dp),intent(in)::Nf
    integer,intent(in)::n,k,flav
    real(dp),dimension(1:3)::CoefSING
    
    SELECT CASE(flav)
    CASE(1)
        CoefSING=C_q_q_plus(n,k,Nf)
    CASE(4)
        CoefSING=C_g_g_plus(n,k,Nf)
    CASE DEFAULT
        write(*,*) ERRORString('No such  SING flav',moduleName)
        stop
    END SELECT
end function CoefSING
