...
type::Xgrid !!!!! name of abstract class
    private
...
SELECT CASE(process0(1))
CASE(2,3)!!!!!!!! Z-boson peak
    weigthFunction=((Q2-MZ2)**2+GammaZ2*MZ2)/(Q2*Q2)
CASE(4,5,6)!!!!!!!! W-boson peak
    weigthFunction=((Q2-MW2)**2+GammaW2*MW2)/(Q2*Q2)
CASE DEFAULT
    weigthFunction=1._dp
end select
...
if(this%outputLevel>2) write(*,*) trim(this%parentName)//": Initializing the structure-function grid for process ("// &
...
v2=TtoV(real(n2,dp),this%v2Min,this%p21)
v3=TtoV(real(n3,dp),this%v3Min,this%p31)
v4=TtoV(real(n4,dp),this%v4Min,this%p41)

this%mainGrid(n1,n3,n3,n4)=this%FtoGrid(v1,v2,v3,v4,this%process0)*weigthFunction(v1,v2,v3,v4,this%process0)
...
if(v1<=this%v1MIN .or. v2<=this%v2MIN .or. v3<=this%v3MIN .or. v4<=this%v4MIN &
    ...
    call this%Warning_Handler%WarningRaise("Point outside of specified grid !")
...
t2=VtoT(v2,this%v2Min,this%p21)
...
p01(:,:,:)=(t-n-1)*p(0,:,:,:)+(t-n-0)*p(1,:,:,:)
p12(:,:,:)=(t-n-2)*p(1,:,:,:)+(t-n-1)*p(2,:,:,:)
p23(:,:,:)=(t-n-3)*p(2,:,:,:)+(t-n-2)*p(3,:,:,:)

p012=(t-n-2)*p01+(t-n-0)*p12
p123=(t-n-3)*p12+(t-n-1)*p23

res=((t-n-3)*p012+(t-n-0)*p123)/6 !!!!! division by 6 is because of 1/2 in the second step and 1/3 in the last step
