...
! integer, dimension(:), allocatable:: hadronsInGRID  !!! table that saves the number of hadrons into plain list
...
!!!! prepare variables
subroutine TMD_Grid_XB_Initialize()
  integer::i
  if(useGrid) then
    xRanges=(/XMin,0.2_dp,1._dp/)
    bRanges=(/bMIN,0.2_dp,bMax/)
    xGridSize=32
    bGridSize=32
    ...
    allocate(gridMain_Ch(1:len(xRanges),0:xGridSize,1:len(bRanges),0:bGridSize,-5:5,1:numberOfHadrons))

  end if
end subroutine TMD_Grid_XB_Initialize


!!!! weight functions for barycentric interpolation formula
pure function barycentricB_x(j,t)
integer,intent(in)::j
real(dp),intent(in)::t
real(dp)::barycentricB_x

barycentricB_x=xNodeFactors(j)/(t-xNodes(j))/sum(xNodeFactors/(t-xNodes))

end barycentricB_x

pure function barycentricB_b(j,t)
integer,intent(in)::j
real(dp),intent(in)::t
real(dp)::barycentricB_b

barycentricB_b=bNodeFactors(j)/(t-bNodes(j))/sum(bNodeFactors/(t-bNodes))
...
subroutine MakeGrid()
  ...
  do h=1,numberOfHadrons
   ...
   !!! it could happen that some values are 0. then I set 1/bE=0 and aE=1
   do iX=0,Nx-1
   do i=-5,5
...
function ExtractFromGrid(x,bT,h)
  ...
  do i=-5,5
   if(ISNAN(ExtractFromGrid(i))) then
    ...
    write(*,*) 'x=',x,'iX=',iX
    write(*,*) 'bT=',bT,' f=',i, ' extraction=',ExtractFromGrid(i)
    write(*,*) 'interI=',interI(0:3,i)
    if(bT>bMax) then
      write(*,*) "Intepolation parameters 1: ", interpolationParameters(1,iX-1,i,h),&
                                              interpolationParameters(1,iX,i,h),&
                                              interpolationParameters(1,iX+1,i,h),&
                                              interpolationParameters(1,iX+2,i,h)
      write(*,*) "Intepolation parameters 2: ", interpolationParameters(2,iX-1,i,h),&
                                              interpolationParameters(2,iX,i,h),&
                                              interpolationParameters(2,iX+1,i,h),&
                                              interpolationParameters(2,iX+2,i,h)
    end if
