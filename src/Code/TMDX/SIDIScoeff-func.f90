...
if(abs(c2_global-1)<toleranceGEN) then
    if(orderH_global>=1) then
        ...
        HardCoefficientSIDIS=HardCoefficientSIDIS+alpha*(-16.946842488404727d0)
    if(orderH_global>=2) then
        HardCoefficientSIDIS=HardCoefficientSIDIS+alpha**2*(-116.50054911601637d0)
    if(orderH_global>=3) then
        HardCoefficientSIDIS=HardCoefficientSIDIS+alpha**3*(-4820.715927678687)
    if(orderH_global>=4) then
        HardCoefficientSIDIS=HardCoefficientSIDIS+alpha**4*(26391.759725461765)
    end if
    end if
    end if
    end if
