...
subroutine BoerMuldersTMDPDF_Initialize(file,prefix)
    ...
    HankelbyLEVIN=LevinIntegrator(path,'*14  ','*F   ',moduleName,outputLevel,TMDtypeN)
    if(makeGrid_inKT) then
         mainKTGrid=ktGrid(path,'*14   ','*F   ',numOfHadrons,includeGluon,moduleName,outputLevel)
    end if
...
function BoerMuldersTMDPDF_TMM_G(x,mu,hadron)
    ...
    BoerMuldersTMDPDF_TMM_G=HankelbyOGATA%Moment_G(F,mu)


    !!! forcefully set =0 below threshold
    if(mu<mBOTTOM) then
    BoerMuldersTMDPDF_TMM_G(5)=0._dp
    BoerMuldersTMDPDF_TMM_G(-5)=0._dp
    end if
    if(mu<mCHARM) then
    BoerMuldersTMDPDF_TMM_G(4)=0._dp
    BoerMuldersTMDPDF_TMM_G(-4)=0._dp
    end if
contains
    function F(b)
...
function BoerMuldersTMDPDF_TMM_X(x,mu,hadron)
    ...
    BoerMuldersTMDPDF_TMM_X=HankelbyOGATA%Moment_X(F,mu)

    !!! forcefully set =0 below threshold
    if(mu<mBOTTOM) then
    BoerMuldersTMDPDF_TMM_X(5)=0._dp
    BoerMuldersTMDPDF_TMM_X(-5)=0._dp
    end if
    if(mu<mCHARM) then
    BoerMuldersTMDPDF_TMM_X(4)=0._dp
    BoerMuldersTMDPDF_TMM_X(-4)=0._dp
    end if
contains
    function F(b)
