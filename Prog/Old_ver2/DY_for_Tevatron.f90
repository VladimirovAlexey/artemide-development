...
!--------------------------------------------------------
!! CALCULATING THE unpolarized DY-cross-section for Tevatron
!!
!!
!!leptons-PT > 25 GeV 
!!|leptons-Eta| < 1.1 
...
!--------------------------------------------------------
! pt-bins
integer,parameter::numPoints=80
integer,parameter::numReplica=300
!
!-------Kinematics definition
real*8,parameter::ptABSOLUTEMAX=40d0
real*8,parameter::s=1960d0**2
integer,dimension(1:3),parameter::proc=(/1,1,6/) !p+pbar->gamma*+Z
real*8,parameter::etamax=1.1d0
real*8,parameter::ptCut=25d0
real*8,parameter::Qmin=66d0
real*8,parameter::Qmax=116d0
!------
real*8::pmin(1:numPoints),pmax(1:numPoints),X(1:numPoints),Xrep(1:numReplica,1:numPoints)
real*8::Xmean(1:numPoints), Xdev(1:numPoints),X1(1:numPoints)
integer::i,j
...
! ! just for check
! do i=1,numPoints
!   call xSec_DY(X1(i),proc,s,(/pmin(i),pmax(i)/),(/Qmin,Qmax/),(/-5d0,5d0/),.false.,(/ptCut,ptCut,-etamax,etamax/))
...
call TMDX_DY_SetCuts(.false.,ptCut,-etamax,etamax)
...
open(7, file='TeV-005-bins', status="replace", action="write")
do i=1,numPoints
  write(7,"(F8.2,F8.2,F12.6)") pmin(i),pmax(i), X(i)
end do
CLOSE (7, STATUS='KEEP')
