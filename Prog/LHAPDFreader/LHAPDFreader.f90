...
logical::OnlyAlphaS !!!!This is a triger. TRUE= only alphaS is loaded and for working
                    !!!!                  FALSE=only PDF is loaded
...
subroutine ParseInfoLine(line)
    ...
    CASE("SetIndex","Authors","FlavorScheme","OrderQCD","ErrorType","DataVersion", &
        "Format", "NumFlavors", "AlphaS_OrderQCD")
    ...
    CASE("AlphaS_Type")
        if(OnlyAlphaS) then
            if(index(linePart2,"ipol")==0) then
                write(*,*) ErrorString('Only "ipol" type of alphaS can be used. Current input is '//trim(linePart2),moduleName)
            stop
            else
    ...
    CASE("AlphaS_Qs")
        if(OnlyAlphaS) then
            ...
            if(outputLevel>=2) write(*,*) "AlphaS_Qs values parsed succesfully"
        else
            if(outputLevel>=3) then
                write(*,*) "Value of "//trim(linePart1)//" in LHAPDF-info :"
                write(*,*) linePart2
    ...
    CASE("AlphaS_Vals")
        if(OnlyAlphaS) then
            ...
            if(outputLevel>=2) write(*,*) "AlphaS_Vals values parsed succesfully"
        else
            if(outputLevel>=3) then
                write(*,*) "Value of "//trim(linePart1)//" in LHAPDF-info :"
                write(*,*) linePart2
    ...
    CASE DEFAULT
        write(*,*) "Unknown entry in LHAPDF-info file :"//linePart1
...
subroutine ReadInfo(name,directory)
    ...
    if(ios /= 0) then
        write(*,*) ErrorString('The info-file is not found at '//trim(path),moduleName)
        stop
    end if
    ...
    if(OnlyAlphaS) then
        if(.not.AlphaStype_IsRecognized) then
            write(*,*) ErrorString('Type of alphaS is not recognized in  '&
            //trim(path),moduleName)
        stop
        end if
        if(.not.allocated(AlphaS_Qs)) then
            write(*,*) ErrorString('AlphaS_Qs variables are missed in '&
            //trim(path),moduleName)
        stop
        end if
        if(.not.allocated(AlphaS_Vals)) then
            write(*,*) ErrorString('AlphaS_Vals variables are missed in '&
            //trim(path),moduleName)
        stop
        end if
...
!         end do
        ...
        if(outputLevel>0) write(*,'(A)',advance="no") color("----- LHAPDF values of alphas from "&
            //trim(name)//" loaded. ",c_yellow)
        if(outputLevel>1) then 
            write(*,*) color("["//trim(ReferenceString)//"]",c_blue)
        else
            write(*,*) " "
        end if
    ...
    else!!!!! check for PDF case
        write(*,*) "THE PDF REDING IS NOT YET DONE!"
        stop
    end if
...
integer::i,j
if(Q<Qmin .or. Q>Qmax) then
    write(*,*) ErrorString('Alphas: Q is outside of Q-range',moduleName)
    stop
end if

logQ=log(Q)

do i=0,size(AlphaS_Qs)-3
