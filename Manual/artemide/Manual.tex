...
This module computes the cross-section of Drell-Yan process, or process with a similar kinematics. The main routine of the whole module is \texttt{xSec\_DY} which returns the cross-section depending on the process (see sec. \ref{sec:process_definition}). The general structure of a cross-section is
\begin{eqnarray}
...
\begin{eqnarray}
\texttt{proc1}=1:~~&&\text{\texttt{xSec\_DY}}\longrightarrow \frac{d\sigma}{dQ^2 dy d\vec q_T^2}~~(\text{DEFAULT})
\\
\texttt{proc1}=2:~~&&\text{\texttt{xSec\_DY}}\longrightarrow \frac{d\sigma}{dQ^2 dx_F d\vec q_T^2}=\frac{1}{2\tau \cosh(y)}\frac{d\sigma}{dQ^2 dy d\vec q_T^2}.
\end{eqnarray}
Other cases (inherited from earlier versions) and marked for depreciation.
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}DY{\_}1pt{\_}Initialize(order)} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
...
\\\hline
\texttt{xSec\_DY(p,process,incCut,CutParam)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section at a point specified by \texttt{DYpoint} p, process and cut variables. 
\\\hline\hline
\end{tabular}
\end{center}
...
\end{eqnarray}
Note, that it is different from result of \texttt{xSec\_DY} by a factor of $(4Qq_T)$. 
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}DY{\_}bin{\_}Initialize(order)} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{Xsec{\_}PTspectrum{\_}Qint{\_}Yint(} \texttt{proc,incCut,CutParam,s,} \texttt{qtMin,qtMax,Qmin,Qmax,ymin,ymax,Num)} & real(dp) &\ref{TMDX:xsec} & Evaluates cross-section completely averaged over the bin for given specification of the process (proc,incCut,CutParam). Size of the bin specified as (qtMin,qtMax),(Qmin,Qmax),(ymin,ymax)]. Num is the minimal number of sections in the integration over $q_T$
\\\hline\hline
\end{tabular}
\end{center}
...
\\\hline
\texttt{xSec\_DY\_List(X,proc,s,qT,Q,y,iC,CutP,Num,doP)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section completely integrated over the bin over the list. 
\\\hline
\texttt{xSec\_DY\_List\_APPROXIMATE(X,proc,s,qT,Q,y,iC,CutP,Num)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section completely integrated over the bin over the list (using several points). 
\\\hline
...
\end{center}
All variables are analogues to those of \texttt{xSec\_DY}, but should come in lists, i.e. \texttt{X} is (1:n), \texttt{process} is (1:n,1:4), \texttt{s} is (1:n), \texttt{qT},\texttt{Q},\texttt{y} are (1:n,1:2), \texttt{includeCuts} is (1:n), \texttt{CutParameters} is (1:n,1:4), \texttt{Num} is (1:n), \texttt{doP} is logic-single. \textbf{The arguments \texttt{Num} and \texttt{doP} are OPTIONAL arguments. The argument \texttt{CutParameters} must be presented}. This command compiled by OPENMP, so runs in parallel on multi-core computers. If \texttt{doP} is \texttt{.true.} than the list is split to runs of identical (except $q_T$) bins and integrated by a faster routine (default=False).
