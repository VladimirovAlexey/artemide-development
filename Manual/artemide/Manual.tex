...
\begin{center}
\includegraphics[width=5cm]{aTMDe_logo}
\\
~
\\
{\Huge \texttt{artemide}} 
...
Artemide uses some commonly defined structures such as conventions on process numbering, specification of kinematics, etc. In this section, we define and describe these conventions.

\subsection{Parton-flavor enumeration}

Artemide uses the LHA-pattern for parton flavor enumeration. I.e. the TMDPDF, TMDFF, and collinear distributions are given by \textit{real(dp)(-5:5)} arrays: \begin{eqnarray}
\textit{real(dp)(-5:5)}=[
\underbrace{\bar b}_{-5},\underbrace{\bar c}_{-4},\underbrace{\bar s}_{-3},\underbrace{\bar u}_{-2},\underbrace{\bar d}_{-1},
\underbrace{g}_{0},
\underbrace{d}_{1},\underbrace{u}_{2},\underbrace{s}_{3},\underbrace{c}_{4},\underbrace{b}_{5}
],
\end{eqnarray}
where the number indicates the FORTRAN index of the array.

\subsection{Hadron enumeration (PDF-case)}

The hadrons are composed of partons, and carry independent non-perturbative structure, which is encoded in the collinear input, and the non-perturbative functions. The type of hadron is specified by an \textit{integer} hadron (=\texttt{h}). At the same time, there are special "hadrons", i.e. systems, that are made as particular combination of ordinary hadrons, such as neutron or deuteron. The current convention within artemide is following
\begin{itemize}
\item There are fundamental hadrons, which are given by \texttt{h}=1,...,9. I.e. maximum 9 such hadrons. By default, \texttt{h}=1 correspond to a proton. Then \texttt{h}=2 correspond to e.g. $\pi$, etc. (As a matter of fact, in very rare cases one needs more than a proton). These hadrons have unique collinear input and unique non-perturbative profiles.
\item The derived hadrons are all derived from \texttt{h}=1, i.e., artemide computes the TMD for $\texttt{h}=1$ and then manipulates it. The numbering for these cases is given below.
\item The negative \texttt{h} corresponds to anti-particle, and has the reversed parton content. i.e.
$$
\texttt{h}= [\bar b,\bar c,\bar s,\bar u,\bar d,g,d,u,s,c,b]~~~\Leftrightarrow~~
-\texttt{h}= [b,c,s,u,d,g,\bar d,\bar u,\bar s,\bar c,\bar b],
$$
for $\texttt{h}>0$. It is applied after the computation of the hadron (so, for example, $\texttt{h}=-11$ corresponds to anti-neutron)
\end{itemize}
\begin{center}
\begin{tabular}{||c|l|c|p{4cm}||}
\hline
~~\texttt{h}~~ & name & Parton transformation rule & Comment
\\\hline
11 & neutron & $[\bar b,\bar c,\bar s,\bar d,\bar u,g,u,d,s,c,b]$&
Proton with $(u\leftrightarrow d,\bar u\leftrightarrow \bar d)$
\\\hline
12 & deuteron & $[\bar b,\bar c,\bar s,\frac{\bar d+\bar u}{2},\frac{\bar d+\bar u}{2},g,\frac{u+d}{2},\frac{u+d}{2},s,c,b]$&
Proton + neutron
\\\hline\hline
$\overbrace{\underbrace{A}_{3\text{dig.}}\underbrace{Z}_{3\text{dig.}}}^{\geqslant 4digits}$ & nuclei $\,^A_ZX$ & $[\bar b,\bar c,\bar s,\frac{Z\bar u+(A-Z)\bar d}{A},\frac{(A-Z)\bar u+Z\bar d}{A},g,\frac{(A-Z)u+Z d}{A},\frac{Z u+(A-Z) d}{A},s,c,b]$& Z Protons + (A-Z) neutrons (normalized)
\\\hline
\multicolumn{4}{||c||}{In particular}
\\
\hline 
64029 & $^{64}_{29}Cu$(copper) & $[\bar b,\bar c,\bar s,\frac{29\bar u+35\bar d}{64},\frac{35\bar u+29\bar d}{64},g,\frac{35 u+29 d}{64},\frac{29u+35 d}{64},s,c,b]$
& Used as E288 target
\\
\hline 
184074 & $^{184}_{74}W$(tungsten) & $[\bar b,\bar c,\bar s,\frac{37\bar u+55\bar d}{92},\frac{55\bar u+37\bar d}{92},g,\frac{55 u+37 d}{92},\frac{37u+55 d}{92},s,c,b]$
& Used as E537 target
\\\hline\hline
\end{tabular}
\end{center}
...
\begin{tcolorbox}
\begin{center}
These definitions are specified in \texttt{aTMDe\_hDef}
\end{center}
\end{tcolorbox}

\subsection{Hadron enumeration (FF-case)}

The same system is implemented in the FF case. However, here, there is one important difference. The hadrons can be composed (i.e. $\pi^++K^+$), so the changes are implemented at the entry of the procedure. The current convention within artemide is following
\begin{itemize}
\item There are fundamental hadrons, which are given by \texttt{h}=1,...,9. I.e. maximum 9 such hadrons. By default, \texttt{h}=1 correspond to a $\pi^+$. Then \texttt{h}=2 corresponds to $K^+$, etc. However, it is not fixed, and should be consistent with input collinear FF. These hadrons have unique collinear input and unique non-perturbative profiles.
\item The derived hadrons are all derived from the special combinations of $(h<10)$. The numbering for these cases is given below.
\item The negative \texttt{h} corresponds to anti-particle, and has the reversed parton content. i.e.
$$
\texttt{h}= [\bar b,\bar c,\bar s,\bar u,\bar d,g,d,u,s,c,b]~~~\Leftrightarrow~~
-\texttt{h}= [b,c,s,u,d,g,\bar d,\bar u,\bar s,\bar c,\bar b],
$$
for $\texttt{h}>0$. It is applied after the computation of hadron (so, for example, $\texttt{h}=-1$ corresponds to $\pi^-$)
\end{itemize}
\begin{center}
...
~~\texttt{h}~~ & name & Parton transformation rule & Comment
\\\hline
12 & $h^+=\pi^++K^+$ & $f_{12}=f_{h=1}+f_{h=2}$ & Charged hadron simulated by two main channels (e.g. for COMPASS)
\\\hline
123 & $h^+=\pi^++K^++p$ & $f_{123}=f_{h=1}+f_{h=2}+f_{h=3}$ & Charged hadron simulated by three channels (e.g. $\pi+K+p$)
\\\hline
10 & $\pi^0=\frac{\pi^++\pi^-}{2}$ & $[\frac{b+\bar b}{2},..., \frac{u+\bar u}{2},\frac{d+\bar d}{2},g,\frac{d+\bar d}{2},...]_{\text{h=1}}$ & Approximate $\pi^0$ \textbf{Derived from h=1}
\\\hline
20 & $K^0=\frac{K^++K^-}{2}$ & $[\frac{b+\bar b}{2},..., \frac{u+\bar u}{2},\frac{d+\bar d}{2},g,\frac{d+\bar d}{2},...]_{\text{h=2}}$ & Approximate $K^0$ \textbf{Derived from h=2}
\\\hline\hline
\end{tabular}
\end{center}
...
\begin{tcolorbox}
\begin{center}
These definitions are specified directly in the FF modules.
\end{center}
\end{tcolorbox}

\subsection{Process numbering}
\red{TO BE DONE}

\subsection{\texttt{(DYpoint)} Drell-Yan reaction kinematic specification}
\label{sec:DYpoint}

The module \texttt{aTMDe\_ptSpec} defines the type \texttt{(DYpoint)} which incorporates and manipulates the variables necessary for efficient operation with Drell-Yan-type reactions. It contains the following public fields
\begin{tabular}{lll}
\texttt{s} &: real(dp)& Mandelshtam variable $s=(p_1+p_2)^2$ (GeV$^2$)
\\
\texttt{Q} &: real(dp)& photon's invariant mass $Q$ (GeV)
\\
\texttt{Q2} &: real(dp)& photon's invariant mass squared $Q^2$ (GeV$^2$)
\\
\texttt{qT} &: real(dp)& absolute value of the photon's transverse momentum $q_T$(GeV)
\\
\texttt{y} &: real(dp)& photon's rapidity $y$
\\
\texttt{expy} &: real(dp)& $e^y$
\\
\texttt{tau} &: real(dp)& $\tau=\sqrt{x_1x_2}=\sqrt{\frac{2q^+q^-}{s}}=\sqrt{\frac{Q^2+\vec q_T^2}{s}}$
\end{tabular}

The constructor for the \texttt{DYpoint} is defined as 
\begin{center}
\texttt{p=DYpoint(qT= .. ,s= .. ,Q= .. ,y= ..)}
\end{center}

Furthermore, it contains the following public functions
\begin{center}
\begin{tabular}{||p{4cm}|c|p{9cm}||}
\hline\hline
Command & ~~Type~~&  Short description
\\\hline
\texttt{updateQ\_DY(p,Q)} & subrout. & Set the value of $Q$, and update all dependent variables within (DYpoint)\texttt{p} by (real(dp)) \texttt{Q}.
\\\hline
\texttt{updateQT\_DY(p,qT)} & subrout. & Set the value of $q_T$, and update all dependent variables within (DYpoint)\texttt{p} by (real(dp)) \texttt{qT}.
\\\hline
\texttt{updateY\_DY(p,y)} & subrout. & Set the value of $y$, and update all dependent variables within (DYpoint)\texttt{p} by (real(dp)) \texttt{y}.
\\\hline
\texttt{coshY()} & real(dp) & Returns $\cosh(y)$.
\\\hline
\texttt{yFromXF(xF,p)} & real(dp) & Computes $y$ associated with a Feynman variable $y=\text{asinh}(\frac{x_F}{2\tau})$ for a given point (DYpoint)\texttt{p} and (real(dp)) \texttt{xF}.
\\\hline\hline
\end{tabular}
\end{center}

\subsection{\texttt{(SIDISpoint)} SIDIS reaction kinematic specification}
\label{sec:SIDISpoint}


The module \texttt{aTMDe\_ptSpec} defines the type \texttt{(SIDISpoint)} which incorporates and manipulates the variables necessary for efficient operation with SIDIS-type reactions. It contains the following public fields
\begin{tabular}{lll}
\texttt{Q} &: real(dp)& photon's invariant mass $Q$ (GeV)
\\
\texttt{Q2} &: real(dp)& photon's invariant mass squared $Q^2$ (GeV$^2$)
\\
\texttt{pPerp} &: real(dp)& absolute value of the product-hadron's transverse momentum $p_\perp$(GeV)
\\
\texttt{M2target} &: real(dp)& value of mass-squared for the target hadron (GeV$^2$)
\\
\texttt{M2product} &: real(dp)& value of mass-squared for the produced hadron (GeV$^2$)
\\
\texttt{sM2} &: real(dp)& value of $(s-M_{\rm target}^2)$ with $s$ being the Mandelstam variable $s$(GeV$^2$)
\\
\texttt{x} &: real(dp)& SIDIS variable $x$
\\
\texttt{z} &: real(dp)& SIDIS variable $z$
\\
\texttt{y} &: real(dp)& SIDIS variable $y=\frac{Q^2}{x(s-M_{\rm target}^2)}$
\\
\texttt{gamma2} &: real(dp)& SIDIS variable $\gamma^2=\big(\frac{2xM}{Q}\big)^2$
\end{tabular}

The constructor for the \texttt{SIDISpoint} is defined as 
\begin{center}
\texttt{p=SIDISpoint(pPerp= .. ,s= .. ,Q= .. ,x= .., z= .., Mtarget= .., Mproduct= ..)}
\end{center}
Note that masses are given in GeV.
...
Command & ~~Type~~&  Short description
\\\hline
\texttt{updateQ\_SIDIS(p,Q)} & subrout. & Set the value of $Q$, and update all dependent variables within (SIDISpoint)\texttt{p} by (real(dp)) \texttt{Q}.
\\\hline
\texttt{updatePperp\_SIDIS(p,pPerp)} & subrout. & Set the value of $p_\perp$ within (SIDISpoint)\texttt{p} by (real(dp)) \texttt{pPerp}.
\\\hline
\texttt{updateX\_SIDIS(p,x)} & subrout. & Set the value of $x$, and update all dependent variables within (SIDISpoint)\texttt{p} by (real(dp)) \texttt{x}.
\\\hline
\texttt{updateZ\_SIDIS(p,z)} & subrout. & Set the value of $z$, and update all dependent variables within (SIDISpoint)\texttt{p} by (real(dp)) \texttt{z}.
\\\hline
\texttt{eps()} & real(dp) & Returns the variable $\varepsilon=\frac{1-y-\frac{\gamma^2 y^2}{4}}{1-y+\frac{y^2}{2}+\frac{\gamma^2 y^2}{4}}$
\\\hline
\texttt{qT()} & real(dp) & Returns the variable $q_T$ (transverse momentum of the photon) for SIDIS, defined as $\vec q_T^2=\frac{\vec p_\perp^2}{z^2} \frac{1+\gamma^2}{1-\gamma^2 \gamma_h^2}$ (with $\gamma_h=m/(zQ)$)
\\\hline
\texttt{ComputeX1Z1qT(x1,z1,qT)} & subrout. & Returns the LP values of $q_T$, $x_1$ and $z_1$ for the SIDIS structure function, with $x_1=-\frac{q^+}{P^+}=x(1-\frac{\vec qT^2}{Q^2})$ and $z_1=\frac{p_h^-}{q^-}=z$.
\\\hline
\texttt{pJacobian()} & real(dp) & Returns the variable $\sqrt{1-\gamma^2 \gamma_h^2-\gamma^2 \frac{\vec p_\perp^2}{(zQ)^2}}$, which is a part of the Jacobian for the transformation from $d^3p_h$ to $dz d\phi d\vec p_\perp^2$.
\\\hline\hline
\end{tabular}
\end{center}
...
\\\hline
4 & \scalebox{0.7}{$\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$} & $f_1$ & $f_1$ & (unpol.)$h_1+h_2\to W^+$ & no & \checkmark & \checkmark
\\\hline
5 & \scalebox{0.7}{$\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$} & $f_1$ & $f_1$ & (unpol.)$h_1+h_2\to W^-$ & no & \checkmark & \checkmark
\\\hline
6 & \scalebox{0.7}{$\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$} & $f_1$ & $f_1$ & (unpol.)$h_1+h_2\to W^\pm$ & no & \checkmark & \checkmark
\\\hline
...
\\\hline \hline
2001 & $\delta_{ff'}|e_f|^2$~~&$f_1$ & $d_1$ & ($F_{UU,T}$)$h_1+\gamma\to h_2$ & no & \checkmark & \checkmark
\\\hline 
2002 & $\delta_{ff'}|e_f|^2$~~&$f^{d}_1$ & $d_1$ & ($F_{UU,T}$)$d^{(h_1)}+\gamma\to h_2$. Here, $d^{(h_1)}$ is the isoscalar target prepared from the hadron $h_1$. I.e. $u,d\to (u+d)/2$. For $h_1$=proton, this simulates the deuteron target. & no & \checkmark & \checkmark
\\\hline
2003 & $\delta_{f\bar f'}|e_f|^2$~~&$f^{n}_1$ & $d_1$ & ($F_{UU,T}$)$n^{(h_1)}+\gamma\to h_2$ Here, $n^{(h_1)}$ is the iso-conjugated target prepared from the hadron $h_1$. I.e. $u\leftrightarrow d$. For $h_1$=proton, this simulates the neutron target. & no & \checkmark & \checkmark
\\\hline \hline
2101 & $\delta_{ff'}|e_f|^2$~~&$f_1$ & $d^{h_{1+2}}_1$ & ($F_{UU,T}$)$h_1+\gamma\to h_{1+2}$, where $h_{1+2}=1+2$. Computed strictly with produced hadron 1 and 2 (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2102 & $\delta_{ff'}|e_f|^2$~~&$f_1$ & $d^{h_{1+2+3}}_1$ & ($F_{UU,T}$)$h_1+\gamma\to h_{1+2+3}$, where $h_{1+2+3}=1+2+3$  Computed strictly with produced hadron 1, 2 and 3 (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2103 & $\delta_{ff'}|e_f|^2$~~&$f^d_1$ & $d^{h_{1+2}}_1$ & ($F_{UU,T}$)$d^{(h_1)}+\gamma\to h_{1+2}$, where $h_{1+2}=1+2$ and d is isoscalar target $u,d\to (u+d)/2$.  Computed strictly with produced hadron 1 and 2 (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2104 & $\delta_{ff'}|e_f|^2$~~&$f^d_1$ & $d^{h_{1+2+3}}_1$ & ($F_{UU,T}$)$d^{(h_1)}+\gamma\to h_{1+2+3}$, where $h_{1+2+3}=1+2+3$ and d is isoscalar target $u,d\to (u+d)/2$.  Computed strictly with produced hadron 1, 2 and 3 (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2105 & $\delta_{ff'}|e_f|^2$~~&$f^n_1$ & $d^{h_{1+2}}_1$ & ($F_{UU,T}$)$n^{(h_1)}+\gamma\to h_{1+2}$, where $h_{1+2}=1+2$ and n is neutron target $(u\leftrightarrow d)$.  Computed strictly with produced hadron 1 and 2 (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2106 & $\delta_{ff'}|e_f|^2$~~&$f^n_1$ & $d^{h_{1+2+3}}_1$ & ($F_{UU,T}$)$n^{(h_1)}+\gamma\to h_{1+2+3}$, where $h_{1+2+3}=1+2+3$ and n is neutron target $(u\leftrightarrow d)$.  Computed strictly with produced hadron 1, 2 and 3 (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2107 & $\delta_{ff'}|e_f|^2$~~&$f_1$ & $d^{h_{1+2}}_1$ & ($F_{UU,T}$)$h_1+\gamma\to h_{1+2}$, where $h_{1+2}=1+2$. Same as 2101 but computed strictly with produced hadron \textbf{3 and 4} (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2108 & $\delta_{ff'}|e_f|^2$~~&$f^d_1$ & $d^{h_{1+2}}_1$ & ($F_{UU,T}$)$d^{(h_1)}+\gamma\to h_{1+2}$, where $h_{1+2}=1+2$ and d is isoscalar target $u,d\to (u+d)/2$.  Same as 2103 but computed strictly with produced hadron \textbf{3 and 4}  (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark & \checkmark
\\\hline
2109 & $\delta_{ff'}|e_f|^2$~~&$f^n_1$ & $d^{h_{1+2}}_1$ & ($F_{UU,T}$)$n^{(h_1)}+\gamma\to h_{1+2}$, where $h_{1+2}=1+2$ and n is neutron target $(u\leftrightarrow d)$.  Same as 2105 but computed strictly with produced hadron \textbf{3 and 4}  (accounts only the sign of $h_2$ argument ignoring its value). & no & \checkmark &  \checkmark
\\\hline \hline
\multicolumn{8}{|c|}{\textbf{For processes 2001$\to$2109 there are alternatives as 20X1$\to$21X9}}
\\\hline
$X=0$ &- &$f_1$ & $d_1$ & Unpolarized contribution to $F_{UU,T}$ (as above) &no & \checkmark & \checkmark
\\\hline
$X=1$ &- &$f_1$ & $d_1$ & Unpolarized contribution to $F_{UU,L}$ (only KPC) &no &  & \checkmark
\\\hline
$X=2$ &- &$f_1$ & $d_1$ & Unpolarized contribution to $F_{UU}^{\cos \phi_h}$ (only KPC) &no &  & \checkmark
\\\hline
$X=3$ &- &$f_1$ & $d_1$ & Unpolarized contribution to $F_{UU}^{\cos 2\phi_h}$ (only KPC) &no &  & \checkmark
\\\hline
$X=4$ &- &$f_1$ & $d_1$ & Unpolarized contribution to $F_{UU,T}+\varepsilon F_{UU,L}$ (only KPC) &no &  & \checkmark
\\\hline \hline
\multicolumn{8}{|c|}{\textbf{For processes 20X1$\to$21X9 there are alternatives as 30X1$\to$31X9}}
\\
\multicolumn{8}{|c|}{They correspond to the Boer-Mulder$\times$Collins contribution (only KPC)}
\\\hline
\end{longtable}
...
\textbf{WARNING!}
\end{center}
This module has been essentially updated/redone in ver.3.05. Now, it is split into sub-modules (allowing for simpler and more detailed operationing), and \textbf{it computes average over bin} (in earlier version it computed integral over the bin). 
\end{tcolorbox}
...
The module has three parts, implemented as independent modules:
\begin{itemize}
\item \texttt{TMDX\_DY\_point}: This module provides the cross-section of Drell-Yan reaction (or analogous) at a single kinematical point.
\item \texttt{TMDX\_DY\_bin}: This module provides the cross-section of Drell-Yan reaction (or analogous), obtained from \texttt{TMDX\_DY\_point} \textbf{averaged} over a bin.
\item \texttt{TMDX\_DY}: This module provides interfaces, checks and parallel machinery for \texttt{TMDX\_DY\_bin} and \texttt{TMDX\_DY\_point}.
\end{itemize}

\subsection{\texttt{TMDX{\_}DY{\_}point} module}

This module computes the cross-section of Drell-Yan process, or process with a similar kinematics. The main routine of this module is \texttt{xSec\_DY\_1pt} which returns the cross-section depending on the process (see sec. \ref{sec:process_definition}). The general structure of a cross-section is
\begin{eqnarray}
d\sigma = prefactor2~\times~(\text{cut-factor for lepton pair})~\times F,
\end{eqnarray}
where
\begin{itemize}
\item $F$ is provided by \texttt{TMDF} or \texttt{TMDF\_KPC} (depending on the setup). It depends on the all-process numbers except the first one.
\item cut-factor is computed within \texttt{LeptonCutsDY}, it represents the correction to the lepton tensor due to fiducial cuts on the lepton pair. It depends on process-type (for angular coefficients the cut is different).
\item prefactor2 accumulates the changes due to the process definition provided in the first number of process.
\end{itemize}

The structure functions computed in \texttt{TMDF} or \texttt{TMDF\_KPC} are organized such that they make cross-section in $(Q,y,q_T)$, so the change of kinematics is accumulated in prefactor2. So for different cases of \texttt{proc1} one gets
\begin{eqnarray}
\texttt{proc1}=1:~~&&\text{\texttt{xSec\_DY\_1pt}}\longrightarrow \frac{d\sigma}{dQ^2 dy d\vec q_T^2}~~(\text{DEFAULT})
\\
\texttt{proc1}=2:~~&&\text{\texttt{xSec\_DY\_1pt}}\longrightarrow \frac{d\sigma}{dQ^2 dx_F d\vec q_T^2}=\frac{1}{2\tau \cosh(y)}\frac{d\sigma}{dQ^2 dy d\vec q_T^2}.
\end{eqnarray}
Other cases are inherited from earlier versions and marked for deprecation.

There are following feature of current implementation
\begin{itemize}
\item The specification of the kinematical point is done by the \texttt{type(DYpoint)}, see section \ref{sec:DYpoint}.
...
\item The flag \texttt{exactX1X2} (INI section \texttt{*9/*C/*p1}) controls how $x_1$ and $x_2$ are computed in the LP version of the factorization. These values correspond
\begin{eqnarray}
\text{\texttt{exactX1X2}=\texttt{.true.}}&\qquad & x_{1,2}=\frac{q_+}{P_+}e^{\pm y}=\sqrt{\frac{Q^2+\vec q_T^2}{s}}e^{\pm y},\qquad \text{(DEFAULT)}
\\
\text{\texttt{exactX1X2}=\texttt{.false.}}&\qquad & x_{1,2}=\frac{Q}{\sqrt{s}}e^{\pm y}.
\end{eqnarray}
In KPC mode, the value of $x$'s unambiguous, and is subject of integral.
\item The flag \texttt{exactZeta} (INI section \texttt{*9/*C/*p}) controls the definition of $\zeta$-scale in the LP version of factorization. 
\begin{eqnarray}
\text{\texttt{exactZeta}=\texttt{.true.}}&\qquad & \zeta=2q^+q^-=Q^2+\vec q_T^2,
\\
\text{\texttt{exactZeta}=\texttt{.false.}}&\qquad & \zeta=Q^2,\qquad \text{(DEFAULT)}.
\end{eqnarray}
In KPC mode, the value of $\zeta$ is defined unambiguously and $\zeta=Q^2$. Note, in all cases $\mu^2=\zeta$.
\item For the definition of \textit{(cuts for lepton pair)}-function see \cite{Scimemi:2017etj}. It is evaluated within module \texttt{LeptonCutsDY.f90}. The presence of cuts, and their parameters are set by the \texttt{SetCutParameters} subroutine.
...
\end{itemize}
\begin{tcolorbox}
The cross-section can be computed by two different means: using LP factorization or using KPC-resummed factorization. The selection is controlled by the \texttt{useKPC} flag in the INI file (section \texttt{*9/*A}). The two modes are mutually exclusive.
\end{tcolorbox}

\begin{center}
List of available commands
\\
\begin{tabular}{||p{6cm}|c|c|p{8cm}||}
\hline\hline
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}DY{\_}1pt{\_}Initialize(file[,prefix])} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{TMDX{\_}DY{\_}1pt{\_}IsInitialized()} & logical & -- & Returns \texttt{.true.} if the module has been initialized.
\\\hline
\texttt{TMDX{\_}DY{\_}1pt{\_}ResetCounters()} & subrout. & -- & Reset intrinsic counters of the module, and prints current statistic on the number of calls.
\\\hline
\texttt{TMDX{\_}DY{\_}1pt{\_}SetScaleVariation(c2)} & subrout. & -- & Set new value for the scale-variation constant $c_2$.
\\\hline
\texttt{xSec\_DY\_1pt(pt,process,} \texttt{incCut,CutParam)} & real(dp) &\ref{TMDX:xsec} & Evaluates cross-section at a point specified by \texttt{DYpoint} pt, process and cut variables.
\\\hline\hline
\end{tabular}
\end{center}

\subsection{\texttt{TMDX{\_}DY{\_}bin} module}

This module computes average over a given kinematical bin. The bin is plain i.e.
\begin{eqnarray}
\texttt{proc1}=1:~~&&\Delta X=\Delta Q \Delta y \Delta q_T = (Q_{\text{max}}-Q_{\text{min}})(y_{\text{max}}-y_{\text{min}})(q_{T\text{max}}-q_{T\text{min}}),
\\
\texttt{proc1}=2:~~&&\Delta X=\Delta Q \Delta x_F \Delta q_T = (Q_{\text{max}}-Q_{\text{min}})(x_{F\text{max}}-x_{F\text{min}})(q_{T\text{max}}-q_{T\text{min}}).
\end{eqnarray}
So, the result of computation is
\begin{eqnarray}
\frac{\Delta\sigma(q_T)}{\Delta X}=\frac{1}{\Delta X}\int_{bin} dX \frac{d\sigma}{dX}.
\end{eqnarray}
So, for small size of a bin the cross-section smoothly turns to a cross-section computed at a point 
\begin{eqnarray}
\texttt{proc1}=1:~~&&\text{\texttt{TMDX\_DY{\_}bin}}\longrightarrow \frac{d\sigma}{dQ dy d|q_T|}
\\
\texttt{proc1}=2:~~&&\text{\texttt{TMDX\_DY{\_}bin}}\longrightarrow \frac{d\sigma}{dQ dx_F d|q_T|}.
\end{eqnarray}
Note, that it is different from the result of \texttt{xSec\_DY\_1pt} by a factor of $(4Qq_T)$.

There are following feature of current implementation
\begin{itemize}
\item If a bin size in any of three variables is small it is considered a point-like in this direction. In this case, the computation of the integral is replaced by the computation of single-point cross-section, which saves a lot of machine-time. It should be also used to compute a cross-section at a point.
\item The minimum bin size limit is specified in the section (9.BIN)
\item There is a significant speed-up in the computation of successive $q_T$-bins. See sec.~\ref{TMDX_DY:pT-partitioning}.
\item Currently, there are two implementations of integration routines, which is dictated by a pre-compile variable \texttt{INTEGRATION\_MODE} (in the header of the code-file). 
\begin{itemize}
\item The case (\texttt{INTEGRATION\_MODE} 1) is fully adaptive (and thus accurate and slow).
\item The case (\texttt{INTEGRATION\_MODE} 2) is fixed-point integration tuned by hands to some reasonable values (and thus not-accurate but fast). In this case artemide show a warning message on start-up, that the approximate mode is used. Exception: the $Z$-boson peak region $[M_Z-3,\,M_Z+3]$~GeV is always integrated with an adaptive Simpson routine to maintain accuracy near the resonance; the fast fixed-point routine is used outside this window.
\end{itemize}
Switch this variable as needed (recompilation of the library is then required).
\end{itemize}
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}DY{\_}bin{\_}Initialize(file[,prefix])} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{TMDX{\_}DY{\_}bin{\_}IsInitialized()} & logical & -- & Returns \texttt{.true.} if the module has been initialized.
\\\hline
\texttt{TMDX{\_}DY{\_}bin{\_}ResetCounters()} & subrout. & -- & Reset warning counters of the bin module.
\\\hline
\texttt{Xsec{\_}PTint{\_}Qint{\_}Yint(} \texttt{proc,incCut,CutParam,s,} \texttt{qtMin,qtMax,Qmin,Qmax,} \texttt{ymin,ymax,Num)} & real(dp) &\ref{TMDX:xsec} & Evaluates cross-section averaged over the full bin. Bin specified by scalar edges (qtMin,qtMax),(Qmin,Qmax),(ymin,ymax). Num is the number of sections in the $q_T$ integration.
\\\hline
\texttt{Xsec{\_}PTspectrum{\_}Qint{\_}Yint(} \texttt{proc,incCut,CutParam,s,} \texttt{qtMin(:),qtMax(:),Qmin,Qmax,} \texttt{ymin,ymax)} & real(dp)(:) &\ref{TMDX:xsec} & Evaluates cross-section averaged over the bin simultaneously for multiple $q_T$ bins using Chebyshev interpolation. \texttt{qtMin} and \texttt{qtMax} are arrays of bin edges; $Q$ and $y$ bounds are shared across all bins.
\\\hline\hline
\end{tabular}
\end{center}

\subsection{\texttt{TMDX{\_}DY} module}

This module is the public-facing wrapper. It initializes both sub-modules (\texttt{TMDX\_DY\_point} and \texttt{TMDX\_DY\_bin}), provides the \texttt{xSec\_DY} and \texttt{xSec\_DY\_List} interfaces used by the end user, and manages the OpenMP parallel dispatch for list evaluation.
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}DY{\_}Initialize(file[,prefix])} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{TMDX{\_}DY{\_}IsInitialized()} & logical & -- & Returns \texttt{.true.} if the module has been initialized.
\\\hline
\texttt{TMDX{\_}DY{\_}ResetCounters()} & subrout. & -- & Reset intrinsic counters of the module (chains into sub-modules; prints statistics if output level is high enough).
\\\hline
\texttt{TMDX{\_}DY{\_}SetScaleVariation(c2)} & subrout. & -- & Set new value for the scale-variation constant $c_2$.
\\\hline
\texttt{xSec\_DY(X,process,s,} \texttt{qT,Q,y,includeCuts,} \texttt{CutParameters,Num)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section averaged over the bin.
\\\hline
\texttt{xSec\_DY\_List(X,process,s,} \texttt{qT,Q,y,includeCuts,} \texttt{CutParameters,Num,doP)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section averaged over the bin for a list of bins; runs in parallel via OpenMP.
\\\hline\hline
\end{tabular}
\end{center}
...
\texttt{call TMDX\_DY{\_}Initialize(file[,prefix])}

where \texttt{file} is the path to the constants file and \texttt{prefix} is an optional string prepended to the path (e.g.\ a directory prefix). The order of used coefficient function is set in \texttt{constant-file} by \texttt{'LO'}($a_s^0$), \texttt{'NLO'}($a_s^1$),\texttt{'NNLO'} or \texttt{'N2LO'}($a_s^2$), \texttt{'N3LO'}($a_s^3$), \texttt{'N4LO'}($a_s^4$). The value in brackets shows the maximum included perturbative order in the hard coefficient function.

\subsection{The parameters of cross-section}
\label{TMDX:setup}


\textbf{Process:} The process is encoded by integer numbers \texttt{(/p1,p2,p3,p4/)} where
\begin{itemize}
...
where
\begin{itemize}
\item \texttt{X} is real(dp), \textbf{output}: value of cross-section.
\item \texttt{process} is integer array \texttt{(p1,p2,p3,p4)}.
\item \texttt{s} is real(dp): Mandelstam variable $s$
\item \texttt{qT} is real(dp) array \texttt{(qtmin,qtmax)}
\item \texttt{Q} is real(dp) array \texttt{(Qmin,Qmax)}
\item \texttt{y} is real(dp) array \texttt{(ymin,ymax)}
\item \texttt{includeCuts} is logical
\item \texttt{CutParameters} is real(dp) array \texttt{(k1,k2,eta1,eta2)} \textbf{OPTIONAL}
\item \texttt{Num} is even integer: number of sections in $q_T$ integral \textbf{OPTIONAL}
\end{itemize}
Note, that optional variables could be omit during the call. 
\end{tcolorbox}

\begin{tcolorbox}
\textbf{\blue{There is also an analogous subroutine that evaluates cross-section by list.}} It is called \texttt{xSec\_DY\_List} and it has the following interface:
\begin{center}
\texttt{xSec\_DY\_List(X,process,s,qT,Q,y,includeCuts,CutParameters,Num,doP)}
\end{center}
All variables are analogous to those of \texttt{xSec\_DY}, but should come in lists, i.e. \texttt{X} is (1:n), \texttt{process} is (1:n,1:4), \texttt{s} is (1:n), \texttt{qT},\texttt{Q},\texttt{y} are (1:n,1:2), \texttt{includeCuts} is (1:n), \texttt{CutParameters} is (1:n,1:4), \texttt{Num} is (1:n), \texttt{doP} is logic-single. \textbf{The arguments \texttt{Num} and \texttt{doP} are OPTIONAL. The argument \texttt{CutParameters} must be present}. This command is compiled with OpenMP and runs in parallel on multi-core computers. If \texttt{doP} is \texttt{.true.} the list is split into runs of identical (except $q_T$) bins and averaged by a faster routine using Chebyshev interpolation in $q_T$ (default=\texttt{.false.}).
\end{tcolorbox}
...
\item In \texttt{INTEGRATION\_MODE} 2 the integration over $q_T$ is not adaptive, since typically $q_T$-bins are rather smooth and the integral is already accurate if approximated by 4--8 points (default minimal value is set in INI-file, and is automatically increased for larger bins). For exceptionally large $q_T$-bins or very rapid behavior, use the optional \texttt{Num} argument to set the number of sections manually. In \texttt{INTEGRATION\_MODE} 1 the $q_T$ integration is also fully adaptive.
...
\subsection{Partitioning of continuous $p_T$ bins integration}
\label{TMDX_DY:pT-partitioning}
...
Given the sequence of bins in $q_T$ $\{\{a_1,b_1\},\{a_2,b_2\},...,\{a_k,b_k\}\}$ with $a_i<b_i$ (all other parameters for cross-section are the same), one can make the interpolation function in the complete range of$(\min(a),\max(b))$, and then integrate the interpolation function. The tests with Chebyshev polynomial interpolation have shown that precision of such procedure is 5-6 digits for N=10 interpolation. The procedure is following:
\begin{enumerate}
\item Fix $N$ (degree-1 of interpolation), and matrix $M$ and nodes $t$,
\begin{eqnarray}
M_{ij}=\frac{2\beta_i\beta_j}{N}\cos\(\frac{ij \pi}{N}\),\qquad t_i=\cos\(\frac{i\pi}{N}\),
\end{eqnarray}
where $\beta_i=1/2$ for $i=1$ or $N$, and $\beta_i=1$ for other $i$.
\item Compute a vector of cross-section at nodes
\begin{eqnarray}
\vec S=\sigma\(\frac{b-a}{2}t_i+\frac{b+a}{2}\),\qquad a=\min(a),\qquad b=\max(b).
\end{eqnarray}
\item Then for each bin the integral over bin reads
\begin{eqnarray}
\int_{a_i}^{b_i} \sigma dq_T= \frac{b-a}{2}\(\vec R\(\frac{2b_i-a-b}{b-a}\)-\vec R\(\frac{2a_i-a-b}{b-a}\)\)\cdot M \cdot \vec S,
\end{eqnarray}
where 
\begin{eqnarray}
\vec R(x)=\{x,\frac{x^2}{2},...,\frac{1}{2}\(\frac{T_{k+1}(x)}{k+1}-\frac{T_{k-1}(x)}{k-1}\),...\}.
\end{eqnarray}
With $T$ being the Chebyshev polynomial.
\end{enumerate}
So, this algorithm requires only $N+1$ evaluations of cross-section.

However, this algorithms works well only in the vicinity of $q_T$-peak, so it is limited to the $q_T$-bins in the range $q_T<q_{T,MAX}$, where $q_{T,MAX}$ is specified in the constants file. In the case of SIDIS, this inequality is $p_T/\langle z\rangle<q_{T\text{MAX}}$.
...
0 & 1 & Test case
\\\hline
1 & $PF_1=\frac{4\pi}{9}\frac{\alpha_{\text{em}}^2(Q)}{sQ^2}|C_V^{DY}(c_2Q)|^2 R \times \text{cut}(q_T)$ & DY-cross-section $\frac{d\sigma}{dQ^2 dy d(q_T^2)}$. cut($q_T$) is the weight for the lepton tensor with fiducial cuts (see sec.2.6 in \cite{Scimemi:2017etj}). Corresponds to DY-cross-section $\frac{d\sigma}{dQ^2 dy d(q_T^2)}$.  The result is in pb/GeV$^2$
\\\hline
2 & $\Big(\frac{2\sqrt{Q^2+\vec q_T^2}}{\sqrt{s}}\cosh y\Big)^{-1} PF_1$ & DY-cross-section $\frac{d\sigma}{dQ^2 dy d(q_T^2)}$. cut($q_T$) is the weight for the lepton tensor with fiducial cuts (see sec.2.6 in \cite{Scimemi:2017etj}).  The result is in pb/GeV$^2$. \textbf{Attention!:} this process corresponds to computation with the $dX=dx_F dQ^2 dq_T^2$ where $x_F=\frac{2\sqrt{Q^2+\vec q_T^2}}{\sqrt{s}}\sinh y$ is Feynman $x$. In this case the integration $y$ is preplaced by the integration over $x_F$ (i.e. limits of integration are treated as for $x_F$).
\\\hline
3 & $\frac{4\pi^2}{3}\frac{\alpha_{\text{em}}(Q)}{s}Br_Z|C_V^{DY}(c_2Q)|^2 R \times \text{cut}(q_T)$ & DY-cross-section $\frac{d\sigma}{dQ^2 dy d(q_T^2)}$ for the Z-boson production in the narrow width approximation. $Br_Z=0.03645$ is Z-boson branching ration to leptons. cut($q_T$) is the weight for the lepton tensor with fiducial cuts (see sec.2.6 in \cite{Scimemi:2017etj}). The result is in pb/GeV$^2$. \red{TO BE DEPRICATED}
\\\hline
4 & $\frac{4\pi^2}{3}\frac{\alpha_{\text{em}}(Q)}{s}Br_W|C_V^{DY}(c_2Q)|^2 R \times \text{cut}(q_T)$ & DY-cross-section $\frac{d\sigma}{dQ^2 dy d(q_T^2)}$ for the W-boson production in the narrow width approximation. $Br_W=0.1086$ is W-boson branching ration to leptons. cut($q_T$) is the weight for the lepton tensor with fiducial cuts (see sec.2.6 in \cite{Scimemi:2017etj}). Use together with \texttt{p3}=13,...,18. In this case, $Q$ is be $M_Z$. The result is in pb/GeV$^2$ \red{TO BE DEPRICATED}
\\\hline
5 & $\frac{2}{\pi}\frac{\pi m_H^2 a_s^2(Q)}{36 s v^2}C_t^2(m_t,Q)|C_g(Q)|^2|A(x_t)|^2$ & Cross-section $\frac{d\sigma}{dy d(q_T^2)}$ for the exclusive Higgs-boson production. The result is in pb/GeV$^2$. For definition of functions see \cite{Ahrens:2009cxz}. \red{TO BE DEPRICATED}
\\\hline\hline
\end{tabular}
...
4 & --- & Absent
\\\hline
5 & $\frac{\pi m_H^2 a_s^2(Q)}{36 s v^2}Q^2 C_t^2(m_t,Q)|C_g(Q)|^2|A(x_t)|^2$ & Cross-section $\frac{d\sigma}{dy d(q_T^2)}$ for the exclusive Higgs-boson production. The result is in pb/GeV$^2$. For definition of functions see \cite{Ahrens:2009cxz}. \red{The correct formula is to be defined, this is LP formula multiplied by large-Q translation factor.} \red{TO BE DEPRICATED}
\\\hline\hline
\end{tabular}
Here $R=0.3893379\cdot 10^9$ the transformation factor from GeV to pb.
\end{center}
...
This module evaluates cross-sections with the SIDIS-like kinematics. I.e. it expects the following kinematic input, $(s,Q,x,z)$ which defines the variables $x$'s, etc. Note, that the structure of the module repeats the structure of \texttt{TMDX{\_}DY} module.

The module has three parts, implemented as independent modules:
\begin{itemize}
\item \texttt{TMDX\_SIDIS\_point}: This module provides the cross-section of SIDIS reaction (or analogous) at a single kinematical point.
\item \texttt{TMDX\_SIDIS\_bin}: This module provides the cross-section of SIDIS reaction (or analogous), obtained from \texttt{TMDX\_SIDIS\_point} \textbf{averaged} over a bin.
\item \texttt{TMDX\_SIDIS}: This module provides interfaces, checks and parallel machinery for \texttt{TMDX\_SIDIS\_bin} and \texttt{TMDX\_SIDIS\_point}.
\end{itemize}


\subsection{\texttt{TMDX{\_}SIDIS{\_}point} module}

This module computes the cross-section of SIDIS, or process with a similar kinematics. The main routine of this module is \texttt{xSec\_SIDIS\_1pt} which returns the cross-section depending on the process (see sec. \ref{sec:process_definition}). The general structure of a cross-section is
\begin{eqnarray}
d\sigma = prefactor2~\times F,
\end{eqnarray}
where
\begin{itemize}
\item $F$ is provided by \texttt{TMDF} or \texttt{TMDF\_KPC} (depending on the setup). It depends on the all-process numbers except the first one.
...
The structure functions computed in \texttt{TMDF} or \texttt{TMDF\_KPC} are organized such that they make cross-section in $(Q,y,q_T)$, so the change of kinematics is accumulated in prefactor2. So for different cases of \texttt{proc1} one gets
\begin{eqnarray}
\texttt{proc1}=1:~~&&\text{\texttt{xSec\_SIDIS\_1pt}}\longrightarrow \frac{d\sigma}{dxdzdQ^2 d\vec p_\perp^2}~~(\text{DEFAULT})
\\
\texttt{proc1}=2:~~&&\text{\texttt{xSec\_SIDIS\_1pt}}\longrightarrow \frac{d\sigma}{dxdzdy d\vec p_\perp^2}=\frac{Q^2}{y}\frac{d\sigma}{dxdzdQ^2 d\vec p_\perp^2},
\\
\texttt{proc1}=3:~~&&\text{\texttt{xSec\_SIDIS\_1pt}}\longrightarrow \frac{d\sigma}{dydzdQ^2 d\vec p_\perp^2}=\frac{x}{y}\frac{d\sigma}{dxdzdQ^2 d\vec p_\perp^2}.
\end{eqnarray}

There are following feature of current implementation
\begin{itemize}
\item The specification of the kinematical point is done by the \texttt{type(SIDISpoint)}, see section \ref{sec:SIDISpoint}.
\item In the current version the scaling variables are set as
\begin{eqnarray}
\mu^2=\zeta_1=\zeta_2=Q^2.
\end{eqnarray}
Currently, it is hard coded and could not be easily modified. However, there is a possibility to vary the value of $\mu$ as $\mu=c_2 Q$, where $c_2$ is variation parameter (see sec.\ref{TMDX:c2}).
\item The flag \texttt{corrQT} (INI section \texttt{*10/*C/*p1}) controls the addition of power-suppressed terms due to non-transversality of the LP hadronic tensor. E.g. the angle-integrated unpolarized part of hadron tensor at LP is $(-g_T^{\mu\nu})$. So being convoluted with lepton tensor it produces extra terms $\sim \frac{\vec p_\perp^2}{Q^2}$, which are included/excluded by this flag. In this example one has (see $c_0^{(un)})$ (\ref{SIDIS:c0un}))
\begin{eqnarray}
\text{\texttt{corrQT}=\texttt{.true.}}&\qquad & (-g_T^{\mu\nu}L_{\mu\nu})=\frac{2Q^2}{1-\varepsilon}\(1+\frac{\vec q_T^2}{Q^2}\frac{\varepsilon -\frac{\gamma^2}{2}}{1+\gamma^2}\),\qquad \text{(DEFAULT)}
\\
\text{\texttt{corrQT}=\texttt{.false.}}&\qquad & (-g_T^{\mu\nu}L_{\mu\nu})=\frac{2Q^2}{1-\varepsilon}.
\end{eqnarray}
In KPC mode, the hadron tensor is exactly transverse and thus the definition is unambiguous.
\item The flag \texttt{exactX1Z1} (INI section \texttt{*10/*C/*p4}) controls how $x_1$ and $z_1$ are computed in the LP version of the factorization. These values correspond
\begin{eqnarray}
\text{\texttt{exactX1Z1}=\texttt{.true.}}&\qquad & x_1=-\frac{q^+}{P^+}=x\(1-\frac{\vec q_T^2}{Q^2}\),\qquad z_1=z,\qquad \text{(DEFAULT)}
\\
\text{\texttt{exactX1Z1}=\texttt{.false.}}&\qquad & x_1=x,~z_1=z.
\end{eqnarray}
In KPC mode, the value of $x$'s unambiguous, and is subject of integral.
\item The flag \texttt{exactZeta} (INI section \texttt{*10/*C/*p5}) controls the definition of $\zeta$-scale in the LP version of factorization.
\begin{eqnarray}
\text{\texttt{exactZeta}=\texttt{.true.}}&\qquad & \zeta=2q^+q^-=Q^2-\vec q_T^2,
\\
\text{\texttt{exactZeta}=\texttt{.false.}}&\qquad & \zeta=Q^2,\qquad \text{(DEFAULT)}.
...
\end{itemize}
\begin{tcolorbox}
The cross-section can be computed by two different means: using LP factorization or using KPC-resummed factorization. The selection is controlled by the \texttt{useKPC} flag in the INI file (section \texttt{*10/*A}). The two modes are mutually exclusive.
\end{tcolorbox}
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}SIDIS{\_}1pt{\_}Initialize} \texttt{(file[,prefix])} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}1pt{\_}IsInitialized()} & logical & -- & Returns \texttt{.true.} if the module has been initialized.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}1pt{\_}ResetCounters()} & subrout. & -- & Reset intrinsic counters of the module, and prints current statistic on the number of calls.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}1pt{\_}SetScaleVariation(c2)} & subrout. & -- & Set new value for the scale-variation constant $c_2$.
\\\hline
\texttt{xSec\_SIDIS\_1pt(p,process)} & real(dp) &\ref{TMDX:xsec} & Evaluates cross-section at a point specified by \texttt{SIDISpoint} \texttt{p} and process.
\\\hline\hline
\end{tabular}
\end{center}

\subsection{\texttt{TMDX{\_}SIDIS{\_}bin} module}

This module computes average over a given kinematical bin. The bin is plain i.e.
\begin{eqnarray}
\texttt{proc1}=1:~~&&\Delta X=\Delta Q \Delta z \Delta x \Delta p_\perp = 
(Q_{\text{max}}-Q_{\text{min}})
(z_{\text{max}}-z_{\text{min}})
(x_{\text{max}}-x_{\text{min}})
(p_{\perp\text{max}}-p_{\perp\text{min}}),
\\
\texttt{proc1}=2:~~&&\Delta X=\Delta y \Delta z \Delta x \Delta p_\perp = 
(y_{\text{max}}-y_{\text{min}})
(z_{\text{max}}-z_{\text{min}})
(x_{\text{max}}-x_{\text{min}})
(p_{\perp\text{max}}-p_{\perp\text{min}}),
\\
\texttt{proc1}=3:~~&&\Delta X=\Delta Q \Delta z \Delta y \Delta p_\perp = 
(Q_{\text{max}}-Q_{\text{min}})
(z_{\text{max}}-z_{\text{min}})
(y_{\text{max}}-y_{\text{min}})
(p_{\perp\text{max}}-p_{\perp\text{min}}),
...
So, for small size of a bin the cross-section smoothly turns to a cross-section computed at a point.
\begin{eqnarray}
\texttt{proc1}=1:~~&&\text{\texttt{TMDX\_SIDIS{\_}bin}}\longrightarrow \frac{d\sigma}{dQ dz dx  d|p_\perp|}
\\
\texttt{proc1}=2:~~&&\text{\texttt{TMDX\_SIDIS{\_}bin}}\longrightarrow \frac{d\sigma}{dy dz dx  d|p_\perp|}=\frac{Q^2}{y}\frac{d\sigma}{dQ^2 dz dx  d|p_\perp|}
\\
\texttt{proc1}=3:~~&&\text{\texttt{TMDX\_SIDIS{\_}bin}}\longrightarrow \frac{d\sigma}{dQ dz dy  d|p_\perp|}=\frac{x}{y}\frac{d\sigma}{dQ dz dy  d|p_\perp|}
\end{eqnarray}
Note, that it is different from the result of \texttt{xSec\_SIDIS\_1pt} (e.g. for \texttt{proc1}=1, by a factor of $4Qp_\perp$).

There are following feature of current implementation
\begin{itemize}
\item If a bin size in any of the four variables is small it is considered point-like in this direction. In this case, the computation of the integral is replaced by the computation of single-point cross-section, which saves a lot of machine-time. It can also be used to compute a cross-section at a point.
\item The minimum bin size limits are specified in the section (10.BIN)
\item There is a significant speed-up in the computation of successive $p_T$-bins. See sec.~\ref{TMDX_DY:pT-partitioning} for an analogous description.
\item Currently, there are two implementations of integration routines, which is dictated by a pre-compile variable \texttt{INTEGRATION\_MODE} (in the header of the code-file). 
\begin{itemize}
\item The case (\texttt{INTEGRATION\_MODE} 1) is fully adaptive (and thus accurate and slow).
\item The case (\texttt{INTEGRATION\_MODE} 2) is fixed-point integration tuned by hands to some reasonable values (and thus not-accurate but fast). In this case artemide show a warning message on start-up, that the approximate mode is used.
\end{itemize}
Switch this variable as needed (recompilation of the library is then required).
\item The mass effects are taken into account in the definitions of external kinematical parameters e.g. $\vec p_\perp$, $\varepsilon$, etc., as well as in the definitions of the cut-region. They are eliminated by setting the input masses to zero. Alternatively, one can set the flags \texttt{includeM1} and/or \texttt{includeM2} to \texttt{.false.}, which sets the masses (of the target and/or product hadrons, correspondingly) to zero at the entry to the module. These flags are controlled by section \texttt{*E/*p1,*p2} of the INI-file.
\end{itemize}
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}SIDIS{\_}bin{\_}Initialize(file[,prefix])} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}bin{\_}IsInitialized()} & logical & -- & Returns \texttt{.true.} if the module has been initialized.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}bin{\_}ResetCounters()} & subrout. & -- & Reset warning counters of the bin module.
\\\hline
\texttt{Xsec{\_}PTint{\_}Qint{\_}Xint{\_}Zint(} \texttt{proc,s,} \texttt{zMin,zMax,xMin,xMax,} \texttt{Qmin,Qmax,ptMin,ptMax,} \texttt{doCut,Cuts,Num,m1,m2)} & real(dp) &\ref{TMDX:xsec} & Evaluates cross-section averaged over the full bin. Bin edges: \texttt{(ptMin,ptMax)}, \texttt{(Qmin,Qmax)}, \texttt{(xMin,xMax)}, \texttt{(zMin,zMax)}. \texttt{m1,m2} are target/product hadron masses (GeV). \texttt{doCut,Cuts=(ymin,ymax,W2min,W2max)} control kinematic cuts. \texttt{Num} is the number of sections in the $p_T$ integration.
\\\hline
\texttt{Xsec{\_}pTspectrum{\_}Qint{\_}Xint{\_}Zint(} \texttt{proc,s,} \texttt{zMin,zMax,xMin,xMax,} \texttt{Qmin,Qmax,ptMin(:),ptMax(:),} \texttt{doCut,Cuts,m1,m2)} & real(dp)(:) &\ref{TMDX:xsec} & Evaluates cross-section averaged over the bin simultaneously for multiple $p_T$ bins using Chebyshev interpolation. \texttt{ptMin} and \texttt{ptMax} are arrays of bin edges; $Q$, $x$, and $z$ bounds are shared across all bins.
\\\hline\hline
\end{tabular}
\end{center}

\subsection{\texttt{TMDX{\_}SIDIS} module}

This module is the public-facing wrapper. It initializes both sub-modules (\texttt{TMDX\_SIDIS\_point} and \texttt{TMDX\_SIDIS\_bin}), provides the \texttt{xSec\_SIDIS} and \texttt{xSec\_SIDIS\_List} interfaces used by the end user, and manages the OpenMP parallel dispatch for list evaluation.
...
Command & ~~Type~~& ~~Sec.~~ & Short description
\\\hline
\texttt{TMDX{\_}SIDIS{\_}Initialize(file[,prefix])} & subrout. & \ref{TMDX:init} & Initialization of module.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}IsInitialized()} & logical & -- & Returns \texttt{.true.} if the module has been initialized.
\\\hline
\texttt{TMDX{\_}SIDIS{\_}ResetCounters()} & subrout. & -- & Reset intrinsic counters of the module (chains into sub-modules; prints statistics if output level is high enough).
\\\hline
\texttt{TMDX{\_}SIDIS{\_}SetScaleVariation(c2)} & subrout. & -- & Set new value for the scale-variation constant $c_2$.
\\\hline
\texttt{xSec\_SIDIS(X,process,s,} \texttt{qT,Q,y,includeCuts,} \texttt{CutParameters,Num)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section averaged over the bin.
\\\hline
\texttt{xSec\_SIDIS\_List(X,process,s,} \texttt{qT,Q,y,includeCuts,} \texttt{CutParameters,Num,doP)} &subrout. &\ref{TMDX:xsec} & Evaluates cross-section averaged over the bin for a list of bins; runs in parallel via OpenMP.
\\\hline\hline
\end{tabular}
\end{center}
...
\begin{center}
\texttt{xSec\_SIDIS\_List(X,process,s,pT,z,x,Q,incC,cuts,masses,doP)}
\end{center}
...
\begin{itemize}
\item \texttt{masses} is optional. If absent replaced by zeros.
\item \texttt{doP} is a single logical value \textit{optional} (default value is specified by INI-file). If \texttt{.true.} the function intents to determine sequences of $p_T$ bins and evaluate integration over them by a single interpolation routine (see sec.\ref{TMDX_DY:pT-partitioning}). The maximum upper limit of a sequence is defined by $p_T/\langle z\rangle<q_{T\text{MAX}}$.
\end{itemize}
\end{tcolorbox}
...
\end{eqnarray}
with $R_t=0.3893379\times 10^9$ the transformation factor from GeV to pb, and (only at LP)
\begin{eqnarray}\label{SIDIS:c0un}
...
\item[\textbf{Ver.3.05}]
\begin{itemize}
	\item \textbf{All TMD-modules} changed the hadron numeration rule (added composite hadrons, and nuclei). Part of definitions are placed in \textbf{aTMDe\_hDef}.
	\item \textbf{TMDX\_SIDIS} The definition of mass-correction is somewhat changed. Now, mass-corrections can be included only into the kinematic variables, and switch off entirely if mass=0. It was not very clear and strict in older versions.
	\item \textbf{TMDX\_SIDIS} is split into three modules \textbf{TMDX\_SIDIS\_point}, \textbf{TMDX\_SIDIS\_bin}, \textbf{TMDX\_SIDIS}. 
	\item \textbf{TMDX\_DY\_point} Fixed error with Jacobian for single-point cross-section with $dx_F$ measure (it was multiplied, instead of divided)
	\item \textbf{TMDX\_DY\_bin} Integration over bin is switched into the averaging over a bin.
	\item \textbf{TMDX\_DY} is split into three modules \textbf{TMDX\_DY\_point}, \textbf{TMDX\_DY\_bin}, \textbf{TMDX\_DY}. Many small fixes (like check of boundaries, etc.).
	\item \textbf{aTMDe\_ptSpec} Implemented.
\end{itemize}
\item[\textbf{Ver.3.04}]
\begin{itemize}
	\item \textbf{SiversTMDPDF, wgtTMDPDF, wglTMDPDF, CollinsTMDFF, BoerMuldersTMDPDF} uniformization of the Kt-integration, and grid (now it is like in uTMDPDF and uTMDFF).
	\item \textbf{Utility modules} uniformization of naming. Found bug in GK2041 (loss of precision) and Stroud35\_56 (incorrect boundary). For all modules made a test codes.
	\item \textbf{\red{All modules}} Passed module by module with Claude, small optimization \& bug fixing. Found 2 bugs (wrong copy-paste for gluon evolution at 3-loop; and incorrect estimation of asymptotic part of Mellin integral in large-x resummed expression), rest bugs are negligible.
\end{itemize}
\item[\textbf{Ver.3.03}]
\begin{itemize}
	\item \textbf{TMDF\_KPC} optimized DY and SIDIS structure function computations (added non-adaptive routines). Added processes with $W$-boson, and SIDIS.
    \item \textbf{grid\_inKT} fixed bug with incorrect extraction from the last segment of the $Q-$grid.
	\item \textbf{TMDX\_DY} \& \textbf{TMDX\_SIDIS} introduced option for evaluation of sequences of $q_T$-bins by a faster method.
	\item \textbf{TMDX\_SIDIS} fixed bug did not set properly the number of processor for parallel computing.
	\item \textbf{TMDX\_DY} fixed bug with incorrect evaluation of integral over $x_F$.
	\item \textbf{TMDX\_DY} \& \textbf{TMDX\_SIDIS} updated the flow of parallel computing, now the computation is ~20-30\% faster for long lists.
	\item \textbf{CollinsTMDFF}: implemented (at all levels)
	\item \textbf{SiversTMDPDF\_OPE} Added griding option for OPE part.
	\item \textbf{aTMDe\_math}: added
...
\item +options regarding the $q_T$-integration in TMDX\_DY section (section 9.B.p6-p8)
\item +options regarding the $q_T$-integration in TMDX\_SIDIS section (section 10.B.p6-p8)
\item +added sections for tw3 grids of wgt and wgl functions(sections 13.G.* and 16.G.*)
\item +added sections for KT grids of Sivers, wgt, and wgl, \textit{it replaces} the older section about Ogata (sections 12.F.*; 13.H.*, and 16.H.*)
\item +added section for the sizes of minimum bin sizes for DY process (sections 9.BIN.*;). Name of the section is strange in order to place it in a convenient place in the file.
\item +added section for the sizes of minimum bin sizes for SIDIS process (sections 9.BIN.*;). Name of the section is strange in order to place it in a convenient place in the file.
\item + the flags for including masses-effects in SIDIS are moved from sec. 10.C.p3-p4 to 10.E.p1-p2.
\end{enumerate}
