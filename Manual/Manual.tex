...
[C\times f](x)&=&x\int_x^1 \frac{dy}{y} C(y,\mathbf{L})f\(\frac{x}{y},\mu_{\text{OPE}}\)
\\
&=&
\int_x^1 dy C(y,\mathbf{L})F\(\frac{x}{y},\mu_{\text{OPE}}\),
\end{eqnarray}
where $F(x)=xf(x)$ and 
\begin{eqnarray}
...
\\\hline
A.p4 & int$>$0 & Number of hadrons/grid to prepare. This must coincide with i.??.p1 (corresponding PDF number in \texttt{QCDinput}). The type of TMD is dictated by the collinear PDF and $f_{\text{NP}}$.
\\\hline
A.p5 & T/F & Trigger the test of the grid (T=run the test). Take the same time as computation of the grid (by single processor). Example Result is shown in sec.\ref{sec:OPE:grid1}. 
\\\hline\hline
C.p1 & real & Tolerance for integration (default =$10^{-6}$)
\\\hline
C.p2 & int & Maximum number of iteration for adaptive integration. (default =$10^{4}$)
\\\hline\hline
