...
\\\hline\hline
\texttt{uTMDPDF\_X0\_AS(x,mu,h,G)} & dp\_real(-5:5) & Computes the AS[$\mathcal{X}_0[f_1]$] as defined in [????.????]. This is asymptotic term required to determine the second moment of TMDPDF. (x,$\mu$)=((real)\texttt{x}, (real)\texttt{mu}), and hadron number (int)\texttt{h}. In the case of \texttt{NA} returns 0. The flag (T/F)\texttt{G} is optional. It indicates the inclusion of gluon into the result. \red{For a moment this function is done only for uTMDPDF, but it will be updated to all twist-TMDs}
\\\hline\hline
\end{tabular}
\end{center}
...
9998 &  &  &  &  & &
\\\hline
1 & $\delta_{\bar f f'}|e_f|^2$ & $f_1$ & $f_1$ & (unpol.)$p+p\to\gamma$ & no & LP/KPC
\\\hline
2 & $\delta_{f f'}|e_f|^2$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to\gamma$ & no & LP/KPC
\\\hline
3 & $\Ds\delta_{\bar f f'}\frac{(1-2|ef|s_w^2)^2-4e_f^2s_w^4}{8s_w^2c_w^2}$ & $f_1$ & $f_1$ & (unpol.)$p+p\to Z$ & no & LP/KPC
\\\hline
4 & $\Ds\delta_{f f'}\frac{(1-2|ef|s_w^2)^2-4e_f^2s_w^4}{8s_w^2c_w^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to Z$ & no & LP/KPC
\\\hline
5 & $\Ds\delta_{\bar f f'}\frac{z_{ll'}z_{ff'}}{\alpha_{\text{em}}^2}$ given in (2.8) of \cite{Scimemi:2017etj} & $f_1$ & $f_1$ & (unpol.)$p+p\to Z+\gamma$ & no & LP/KPC
\\\hline
6 & $\Ds\delta_{f f'}\frac{z_{ll'}z_{ff'}}{\alpha_{\text{em}}^2}$ given in (2.8) of \cite{Scimemi:2017etj} & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to Z+\gamma$ & no & LP/KPC
\\\hline
7 & $\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$ & $f_1$ & $f_1$ & (unpol.)$p+p\to W^+$ & no & LP/KPC
\\\hline
8 & $\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$ & $f_1$ & $f_1$ & (unpol.)$p+p\to W^-$ & no & LP/KPC
\\\hline
9 & $\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$ & $f_1$ & $f_1$ & (unpol.)$p+p\to W^\pm$ & no & LP/KPC
\\\hline
10 & $\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to W^+$ & no & LP/KPC
\\\hline
11 & $\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to W^-$ & no & LP/KPC
\\\hline
12 & $\Ds \frac{1}{4s_w^2}\frac{|V_{f f'}|^2}{4s_w^2}\frac{Q^4}{(Q^2-M_W^2)^2+\Gamma_W^2M_W^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to W^\pm$ & no & LP/KPC
\\\hline
13 & $\Ds \frac{|V_{f f'}|^2}{4s_w^2}$  & $f_1$ & $f_1$ & (unpol.)$p+p\to W^+$ (for narrow-width approx.) & no & LP/KPC
\\\hline
14 & $\Ds \frac{|V_{f f'}|^2}{4s_w^2}$  & $f_1$ & $f_1$ & (unpol.)$p+p\to W^-$ (for narrow-width approx.) & no & LP/KPC
\\\hline
15 & $\Ds \frac{|V_{f f'}|^2}{4s_w^2}$  & $f_1$ & $f_1$ & (unpol.)$p+p\to W^\pm$ (for narrow-width approx.) & no & LP/KPC
\\\hline
16 & $\Ds \frac{|V_{f f'}|^2}{4s_w^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to W^+$ (for narrow-width approx.) & no & LP/KPC
\\\hline
17 & $\Ds \frac{|V_{f f'}|^2}{4s_w^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to W^-$ (for narrow-width approx.) & no & LP/KPC
\\\hline
18 & $\Ds \frac{|V_{f f'}|^2}{4s_w^2}$ & $f_1$ & $f_1$ & (unpol.)$p+\bar p\to W^\pm $ (for narrow-width approx.) & no & LP/KPC
\\\hline
20 & $\Ds f_g(x_1)f_g(x_2)+h^\perp_{1}(x_1)h^\perp_{1}(x_2)$ & - & - & (unpol.)$h+h\to H$ Elementary Higgs production & yes & LP/KPC
\\\hline
21 & $\Ds f_g(x_1)f_g(x_2)$ & $f_1$ & $f_{1}$ & (unpol.)$h+h\to H$ Elementary Higgs production. Only unpol.TMDPDF term & yes & LP/KPC
\\\hline
22 & $\Ds h^\perp_{1g}(x_1)h^\perp_{1g}(x_2)$ & $h^\perp_{1}$ & $h^\perp_{1}$ & (unpol.)$h+h\to H$ Elementary Higgs production. Only linearly pol.TMDPDF term & yes & LP/KPC
\\\hline
101 & $\delta_{\bar f f'}|e_f|^2$ & $f^h_1$ & $f_1$ & (unpol.)$h+p\to\gamma$ (with $h=2$) & no & LP
\\\hline
102 & $\delta_{f f'}|e_f|^2$ & $f^h_1$ & $f_1$ & (unpol.)$h+\bar p\to\gamma$(with $h=2$) & no & LP
\\\hline
103 & $\delta_{f f'}|e_f|^2$ & $f^h_1$ & $f_1$ & (unpol.)$\bar h+p\to\gamma$(with $h=2$). Note, that it is equal 102 & no & LP
\\\hline
104 & $\delta_{\bar f f'}|e_f|^2$ & $f^h_1$ & $f_1$ & (unpol.)$\bar h+\bar p\to\gamma$ (with $h=2$). Note, that it is equal 101 & no & LP
\\\hline
1001 & $\Ds R^{Cu}_{\bar f f'}e_f e_{f'} $ see (3.1) of \cite{Scimemi:2017etj} & $f_1$ & $f^{Cu}_1$ & (unpol.)$p+Cu\to \gamma$ where $h$ is proton (roughly simulates isostructure of Cu, used to describe E288 experiment in \cite{Scimemi:2017etj}) & no & LP/KPC
\\\hline
1002 & $\Ds R^{^2H}_{\bar f f'}e_f e_{f'} $ see (3.1) of \cite{Scimemi:2017etj} with $Z=1$ and $A=2$ & $f_1$ & $f^{^2H}_1$ & (unpol.)$p+^2H\to \gamma$ where $h$ is proton (roughly simulates isostructure of $^2H$, used to describe E772 experiment)& no & LP/KPC
\\\hline
1003 & $\Ds R^{^W}_{f f'}e_f e_{f'} $ see (3.1) of \cite{Scimemi:2017etj} with $Z=74$ and $A=183$ & $f_1$ & $f^{W}_1$ & (unpol.)$\bar p+W\to \gamma$, where $h$ is proton (roughly simulates isostructure of $W$(tungsten), used to describe E537 experiment)& no & LP/KPC
\\\hline
1004 & $\Ds R^{^W}_{\bar f f'}e_f e_{f'} $ see (3.1) of \cite{Scimemi:2017etj} with $Z=74$ and $A=183$ & $f_1$ & $f^{W}_1$ & (unpol.)$h+W\to \gamma$, where $h$ is $\pi^-$ (roughly simulates isostructure of $\pi^-+W$(tungsten), used to describe E537 experiment)& no & LP/KPC
\\\hline \hline
...
\end{eqnarray}
where $F$ is defined in (\ref{TMDF:F}) or (\ref{KPC:main}). There are following feature of current implementation
\begin{itemize}
